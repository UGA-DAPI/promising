...
*/
function promising_tree_delete($id, $table, $istree = 1){
		promising_tree_updateordering($id, $table, $istree);
...
function promising_tree_delete_messages($id, $table, $istree = 1){
		promising_tree_updateordering_messages($id, $table, $istree);
...
function promising_tree_updateordering_messages($id, $table, $istree){
...
function tree_copy_rec($table, $src, $into, $srcisroot = false){
	...
	if (!$srcisroot){
		$srcrec = $DB->get_record("promising_$table", array('id' => $src));
		unset($srcrec->id);
		$srcrec->fatherid = $into;
		$dstordering = 0 + $DB->get_field("promising_$table", 'MAX(ordering)', array('fatherid' => $into));
		$srcrec->ordering = $dstordering + 1;
		$copiedid = $DB->insert_record("promising_$table", $srcrec);
	} else {
		// fake copied to copy into $into
		$copiedid = $into;
	}
	// get childs and recurse
	if ($childs = $DB->get_records("promising_$table", array('fatherid' => $src))){
...
*/
function promising_tree_reorderlevel($parentid, $table, $projectid = 0, $groupid = 0){
	global $DB, $OUTPUT;
...
function promising_tree_reorderlevel_messages($parentid, $table, $projectid = 0, $groupid = 0){
	global $DB, $OUTPUT;
...
*/
function promising_tree_updateordering($id, $table, $istree){
...
* @return void
*/
function promising_tree_up($project, $group, $id, $table, $istree = 1){
	global $CFG, $DB;
	...
	if ($istree)
	    promising_tree_reorderlevel($res->fatherid, $table, $project->id, $group);
}
function promising_tree_up_messages($project, $group, $id, $table, $istree = 1){
	global $CFG, $DB;
	...
	if ($istree)
	    promising_tree_reorderlevel_messages($res->parent, $table, $project->id, $group);
}
/**
...
*/
function promising_tree_down(&$project, $group, $id, $table, $istree = 1){
	global $DB;
...
function promising_tree_down_messages(&$project, $group, $id, $table, $istree = 1){
	global $DB;
...
*/
function promising_tree_left(&$project, $group, $id, $table){
	global $DB;
...
* @param string $table the table-tree name
*/
function promising_tree_right(&$project, $group, $id, $table){
	global $DB;
    ...
    promising_tree_reorderlevel($fatherid, $table, $project->id, $group);
	...
	if( 1 < $ordering ){
		...
		$maxordering = promising_tree_get_max_ordering($project->id, $group, $table, true, $newfatherid);
...
*/
function promising_get_tree_options($table, $projectid, $groupid, $fatherid = 0, $ordering = ''){
    global $DB;
    ...
    if ($elements = $DB->get_records_sql($sql)){
        foreach($elements as $anElement){
            ...
            $collected[] = $anElement;
            $collected = array_merge($collected, promising_get_tree_options($table, $projectid, $groupid, $anElement->id, $anElement->ordering));
        }
    }
    return $collected;
}
...
*/
function promising_get_subtree_list($table, $id){
	global $DB;
    ...
    if (is_array($res)){
        foreach(array_keys($res) as $aSub){
            $ids[] = $aSub;
            $subs = promising_get_subtree_list($table, $aSub);
...
*/
function promising_count_subs($table, $id){
    global $DB;
...
*/
function promising_count_leaves($table, $id, $returnList=false){
    global $DB;
    if (promising_count_subs($table, $id) == 0){
	...
	if ($ress){
        foreach($ress as $res){
            if ($returnList){
                $leafIds = array_merge($leafIds, promising_count_leaves($table, $res->id, true));
            } else {
                $leaves += promising_count_leaves($table, $res->id, false);
            }
        }
    }
...
*/
function promising_tree_propagate_up($table, $field, $id, $function = '~', $byFather = false){
    global $DB;
    if (!$byFather){
    ...
    if ($fatherid){
        ...
        promising_tree_propagate_up($table, $field, $fatherid, $function);
    }
}
...
* when propagating after a record is deleted
*/
function promising_tree_propagate_down(&$project, $table, $field, $fatherid = 0, $function = '~'){
	global $DB;
    ...
    if($res = $DB->get_records_select_menu($table, " fatherid = ? AND projectid = ? ", array($fatherid, $project->id), 'id', "id, $field")){
        foreach(array_keys($res) as $resid){
            promising_tree_propagate_down($project, $table, $field, $resid, $function);
        }
        // calculate mathematic meaning
        if ($fatherid != 0){
...
*/
function promising_tree_get_upper_branch($table, $id, $includeStart = false, $returnordering = false, $reverse = true){
    global $DB;
...
*/
function promising_tree_copy_set($set, $fromtable, $totable, $fields = 'description,format,abstract,projectid,groupid,ordering', $autobind = false, $bindtable = ''){
    ...
    function place_in_tree($table, &$flatitemids, &$items, $itemid){
        $branch = promising_tree_get_upper_branch($table, $itemid, false, false, false);
    ...
    function insert_tree($projectid, $group, $table, &$set, $autobind = false, $bindtable = ''){
        ...
        for($i = 0 ; $i < count($set) && $i < 1000; $i++){
            ...
            if ($set[$setkeys[$i]]->fatherid == 0){
                $position = promising_tree_get_max_ordering($projectid, $group, $table);
                // remove child as not member in the db data record
            } else {
                $position = promising_tree_get_max_ordering($projectid, $group, $table, true, $set[$setkeys[$i]]->fatherid);
            }
            ...
            if ($autobind){
            	switch ($bindtable){
            		case 'promising_spec_to_req' :
            			if ($table == 'promising_specification'){
            				$t1 = 'reqid';
            				$t2 = 'specid';
            			} else {
            				$t1 = 'specid';
        			...
        			break;
            		case 'promising_task_to_deliv' :
            			if ($table == 'promising_deliverable'){
            				$t1 = 'taskid';
            				$t2 = 'delivid';
            			} else {
            				$t1 = 'delivid';
...
*/
function promising_tree_get_max_ordering($projectid, $group, $table, $istree = false, $fatherid = 0){
    global $DB;
...
function promising_tree_get_max_ordering_message($projectid, $group, $table, $istree = false, $parent = 0){
    global $DB;
...
*/
function promising_tree_get_tree($table, $projectid, $groupid, &$tree, $fatherid = 0){
    global $DB;    
    static $deepness = 0;
    static $nodecode;
    ...
    if ($records){
        foreach($records as $key => $record){
            ...
            promising_tree_get_tree($table, $projectid, $groupid, $tree, $record->id);
            $nodecode = $oldnode;
            $deepness--;
        }
    }
...
*/
function promising_tree_get_list($table, $projectid, $groupid, &$list){
    global $DB;
