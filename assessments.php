<?php
    ...
    */
    ...
    if (!has_capability('mod/promising:gradeproject', $context)){
    ...
    if (!groups_get_activity_groupmode($cm, $project->course)){
        ...
        $groupStudents = get_users_by_capability($context, 'mod/promising:canbeevaluated', 'id,firstname,lastname,email,picture', 'lastname');
    } else {
        $groupmembers = groups_get_members($currentGroupId);
        foreach($groupmembers as $amember){
            if (!has_capability('mod/promising:canbeevaluated', $context, $amember->id)) continue;
            $groupStudents[] = clone($amember);
        }
    }
    if (!isset($groupStudents) || count($groupStudents) == 0 || empty($groupStudents)){
        echo $OUTPUT->box(get_string('noonetoassess', 'promising'), 'center', '70%');
        return;
    } else {
       ...
       echo '<center><i>'.get_string('evaluatingforusers', 'promising') .' : </i> '. implode(',', $studentListArray) . '</center><br/>';
    }
    if ($work == 'regrade'){
        $autograde = promising_autograde($project, $currentGroupId);
        if ($project->grade > 0){
            unset($assessment);
            $assessment->id = 0;
            ...
            if ($oldrecord = $DB->get_record_select('promising_assessment', "projectid = {$project->id} AND userid = {$aStudent->id} AND itemid = 0 AND itemclass='auto'")){
                $assessment->id = $oldrecord->id;
                $DB->update_record('promising_assessment', $assessment);
                add_to_log($course->id, 'promising', 'grade', "view.php?id=$cm->id&view=view_summary", $project->id, $cm->id, $aStudent->id);
            } else {
                $DB->insert_record('promising_assessment', $assessment);
            }
        }
    }
    echo $OUTPUT->box_end();
    ...
    if ($work == 'dosave'){
        ...
        foreach($groupStudents as $aStudent){
            ...
            if ($project->autogradingenabled){
                ...
                if ($oldrecord = $DB->get_record_select('promising_assessment', "projectid = {$project->id} AND userid = {$aStudent->id} AND itemid = '{$assessment->itemid}' AND itemclass='{$assessment->itemclass}'")){
                    $assessment->id = $oldrecord->id;
                    $DB->update_record('promising_assessment', $assessment);
                    add_to_log($course->id, 'promising', 'grade', "view.php?id={$cm->id}&view=view_summary&group={$currentGroupId}", $project->id, $cm->id, $aStudent->id);
                } else {
                    $DB->insert_record('promising_assessment', $assessment);
                }
            }
            ...
            foreach($teacherGrades as $aGradeKey){
                ...
                if (!empty($grade)) $assessment->grade = $grade;
                if ($oldrecord = $DB->get_record_select('promising_assessment', "projectid = {$project->id} AND userid = {$aStudent->id} AND itemid = {$assessment->itemid} AND itemclass='{$assessment->itemclass}' {$criterionClause}")){
                    $assessment->id = $oldrecord->id;
                    $DB->update_record('promising_assessment', $assessment);
                } else {
                    $DB->insert_record('promising_assessment', $assessment);
            ...
            foreach($freeGrades as $aGradeKey){
                ...
                $grade = optional_param($aGradeKey, '', PARAM_INT);
                if (!empty($grade)) $assessment->grade = $grade;
                if ($oldrecord = $DB->get_record_select('promising_assessment', "projectid = {$project->id} AND userid = {$aStudent->id} AND itemclass = 'free' AND itemid = 0 AND criterion = {$assessment->criterion}")){
                    $assessment->id = $oldrecord->id;
                    $DB->update_record('promising_assessment', $assessment);
                } else {
                    $DB->insert_record('promising_assessment', $assessment);
    ...
    elseif ($work == 'doerase'){
        foreach($groupStudents as $aStudent){
            $DB->delete_records('promising_assessment', array('projectid' => $project->id, 'userid' => $aStudent->id));
            add_to_log($course->id, 'promising', 'grade', "view.php?id=$cm->id&view=view_summary&group={$currentGroupId}", 'erase', $cm->id, $aStudent->id);
            add_to_log($course->id, 'promising', 'grade', "view.php?id=$cm->id&view=view_summary&group={$currentGroupId}", 'erase', $cm->id);
        }
    }
    if ($project->teacherusescriteria){
        $freecriteria =  $DB->get_records_select('promising_criterion', "projectid = {$project->id} AND isfree = 1");
        $criteria = $DB->get_records_select('promising_criterion', "projectid = {$project->id} AND isfree = 0");
        if (!$criteria && !$freecriteria){
            echo $OUTPUT->box(format_text(get_string('cannotevaluatenocriteria','promising'), FORMAT_HTML), 'center');
            return;
        }
    }
    $canGrade = false;
    ...
    <table width="80%">
    <?php
        $milestones = $DB->get_records_select('promising_milestone', "projectid = {$project->id} AND groupid = {$currentGroupId}");
        $grades = $DB->get_records_select('promising_assessment', "projectid = {$project->id} AND groupid = {$currentGroupId} GROUP BY itemid,criterion,itemclass");
        $gradesByClass = array();
        ...
        if($milestones && (!$project->teacherusescriteria || $criteria)){
            ...
            echo $OUTPUT->heading(get_string('itemevaluators', 'promising'));
            echo "</td></tr>";
            foreach($milestones as $aMilestone){
                echo "<tr valign=\"top\"><td align=\"left\"><b>";
                echo get_string('evaluatingfor','promising')." M.{$aMilestone->ordering} {$aMilestone->abstract}</b>";
                echo "</td></tr><tr><td>";
                if (!$project->teacherusescriteria){
                    $teachergrade = @$gradesByClass['milestone'][$aMilestone->id][0];
                    echo get_string('teachergrade','promising').' ';
        ...
        if ($project->autogradingenabled || $project->teacherusescriteria){
            ...
            echo $OUTPUT->heading(get_string('globalevaluators', 'promising'));
            echo "</td></tr>";
            $canGrade = true;
        }
        if ($project->autogradingenabled){
            $autograde = @$gradesByClass['auto'][0][0];
            echo '<tr><td align="left">'.get_string('autograde','promising').'</td><td align="left">';
            echo make_grading_menu($project, 'autograde', $autograde, true);
            echo " <a href=\"?work=regrade&amp;id={$cm->id}\">".get_string('calculate','promising').'</a></td></tr>';
        }
        if ($project->teacherusescriteria){
            if (@$freecriteria){
        ...
        if (!$canGrade){
            echo $OUTPUT->box(get_string('cannotevaluate', 'promising'), 'center', '70%');
        }
    ?>
    </table>
    <br/>
    <br/>
    <input type="button" name="go_btn" value="<?php print_string('updategrades', 'promising') ?>" onclick="senddata('save')" />
    <input type="button" name="erase_btn" value="<?php print_string('cleargrades', 'promising') ?>" onclick="senddata('erase')" />
    </form>
    </center>