...
/**
 * Define all the backup steps that will be used by the backup_promising_activity_task
 */

/**
 * Define the complete label structure for backup, with file and id annotations
 */
class backup_promising_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $promising = new backup_nested_element('promising', array('id'), array(
        ...
        $promising->add_child($globaldomains);
        $globaldomains->add_child($globalqualifier);

        $promising->add_child($heading);

        $promising->add_child($requirements);
        $requirements->add_child($requirement);

        $promising->add_child($specifications);
        $specifications->add_child($specification);

        $promising->add_child($tasks);
        $tasks->add_child($task);

        $promising->add_child($milestones);
        $milestones->add_child($milestone);

        $promising->add_child($delivs);
        $delivs->add_child($delivs);

        $promising->add_child($validationsessions);
        ...
        $promising->add_child($assessments);
        $assessments->add_child($assessment);

        $promising->add_child($criteria);
        $criteria->add_child($criterion);

        // Define sources
        $promising->set_source_table('promising', array('id' => backup::VAR_ACTIVITYID));
        $globalqualifier->set_source_table('promising_qualifier', array('projectid' => 0));

        if ($userinfo) {
            $heading->set_source_table('promising_heading', array('projectid' => backup::VAR_ACTIVITYID));
            $requirement->set_source_table('promising_requirement', array('projectid' => backup::VAR_ACTIVITYID));
            $specification->set_source_table('promising_specification', array('projectid' => backup::VAR_ACTIVITYID));
            $task->set_source_table('promising_task', array('projectid' => backup::VAR_ACTIVITYID));
            $milestone->set_source_table('promising_milestone', array('projectid' => backup::VAR_ACTIVITYID));
            $deliverable->set_source_table('promising_deliverable', array('projectid' => backup::VAR_ACTIVITYID));

            $spectoreq->set_source_table('promising_spec_to_req', array('projectid' => backup::VAR_ACTIVITYID));
            $tasktospec->set_source_table('promising_task_to_spec', array('projectid' => backup::VAR_ACTIVITYID));
            $tasktodeliv->set_source_table('promising_task_to_deliv', array('projectid' => backup::VAR_ACTIVITYID));
            $tasktdep->set_source_table('promising_task_dependency', array('projectid' => backup::VAR_ACTIVITYID));

            $assessment->set_source_table('promising_assessment', array('projectid' => backup::VAR_ACTIVITYID));

            $validationsession->set_source_table('promising_valid_session', array('projectid' => backup::VAR_ACTIVITYID));
            $validationresult->set_source_table('promising_valid_state', array('projectid' => backup::VAR_ACTIVITYID, 'validationsessionid' => backup::VAR_PARENTID));
        	...
        	$qualifier->set_source_table('promising_qualifier', array('projectid' => 0));
        }
        ...
        $promising->annotate_files('mod_promising', 'intro', null); // This file area hasn't itemid
        $promising->annotate_files('mod_promising', 'abstract', null); // This file area hasn't itemid
        $promising->annotate_files('mod_promising', 'rationale', null); // This file area hasn't itemid
        $promising->annotate_files('mod_promising', 'environment', null); // This file area hasn't itemid
        $promising->annotate_files('mod_promising', 'requirementdescription', 'id'); 
        $promising->annotate_files('mod_promising', 'specificationdescription', 'id'); 
        $promising->annotate_files('mod_promising', 'taskdescription', 'id'); 
        $promising->annotate_files('mod_promising', 'milestonedescription', 'id'); 
        $promising->annotate_files('mod_promising', 'deliverabledescription', 'id'); 
        $promising->annotate_files('mod_promising', 'localfile', 'id'); 
        $promising->annotate_files('mod_promising', 'xslfilter', null); // This file area hasn't itemid
        $promising->annotate_files('mod_promising', 'cssfilter', null); // This file area hasn't itemid

        // Return the root element (promising), wrapped into standard activity structure
        return $this->prepare_activity_structure($promising);
    }
}
