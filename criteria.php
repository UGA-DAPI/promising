<?php
	...
	*/
    ...
    if ($work == 'update'){
        ...
        if ($DB->get_record('promising_criterion', array('id' => $criterion->id))){
            $DB->update_record('promising_criterion', $criterion);
        } else {
            $DB->insert_record('promising_criterion', $criterion);
        }
        ...
        $criterion->isfree = optional_param('isfree', 0, PARAM_INT);
        if ($DB->get_record('promising_criterion', array('id' => $criterion->id))){
            $DB->delete_records('promising_criterion', array('id' => $criterion->id));
            $DB->delete_records('promising_assessment', array('projectid' => $project->id, 'criterion' => $criterion->id));
        }
    }
    if ($work == 'dodelete'){
    	$criterion = new StdClass;
        $criterion->id = required_param('item', PARAM_INT);
        $criterion->isfree = optional_param('isfree', 0, PARAM_INT);
        echo $OUTPUT->heading(get_string('confirmdeletecriteria', 'promising')); 
        echo $OUTPUT->box(get_string('criteriadeleteadvice', 'promising'), 'center', '70%');
    ?>
...
/// Criteria update form ***************************************************
    ...
    <table width="80%" cellspacing="10" style="padding : 10px">
        <tr>
            <td valign="top">
                <?php 
                echo $OUTPUT->heading(get_string('freecriteriaset', 'promising') . ' ' . $OUTPUT->help_icon('freecriteriaset', 'promising', false)) 
                ?>
                <script type="text/javascript">
                //<![CDATA[
                function senddatafree(){
                    if (document.forms['freecriteriaform'].criterion == ''){
                        alert('<?php print_string('emptycriterion','promising') ?>');
                        return;
                    }
                ...
                <form name="freecriteriaform" method="post" action="view.php">
                    ...
                    if($freeCriteria){
                        foreach($freeCriteria as $aCriterion){
                            $links = "<a href=\"view.php?id={$cm->id}&amp;work=dodelete&amp;isfree=1&amp;item={$aCriterion->id}\"><img src=\"{$CFG->pixpath}/t/delete.gif\" title=\"".get_string('deletecriteria', 'promising')."\" border=\"0\" /></a>";
                            $links .= "<a href=\"javascript:changefree('{$aCriterion->id}','{$aCriterion->criterion}','{$aCriterion->label}','{$aCriterion->weight}')\"><img src=\"{$CFG->pixpath}/t/edit.gif\" title=\"".get_string('editcriteria', 'promising')."\" border=\"0\" /></a>";
                    ...
                    <table>
                        <tr>
                            <td align="right">
                                <?php print_string('criterion', 'promising') ?>&nbsp;
                            </td>
                            <td> 
                                <input type="text" name="criterion" value="" /> 
                                <?php $OUTPUT->help_icon('criterion', 'promising') ?>
                            </td>
                        </tr>
                        <tr>
                            <td align="right">
                                <?php print_string('label', 'promising') ?>&nbsp;
                            </td>
                            <td> 
                                <input type="text" name="label" value="" /> 
                                <?php $OUTPUT->help_icon('label', 'promising') ?>
                            </td>
                        </tr>
                        <tr>
                            <td align="right">
                                <?php print_string('weight', 'promising') ?>&nbsp;
                            </td>
                            <td> 
                                <input type="text" name="weight" value="" /> 
                                <?php $OUTPUT->help_icon('weight', 'promising') ?>
                            </td>
                        </tr>
                        <tr>
                            <td>
                            ...
                            <td align="right">
                                <input type="button" name="go_btn" value="<?php print_string('save', 'promising') ?>" onclick="senddatafree('save')" />
                            </td>
                        </tr>
                    </table>
            ...
            <td valign="top">
                <?php 
                echo $OUTPUT->heading(get_string('itemcriteriaset', 'promising') . ' ' . $OUTPUT->help_icon('itemcriteriaset', 'promising', false)) 
                ?>
                <script type="text/javascript">
                //<![CDATA[
                function senddata(){
                ...
                <form name="criteriaform" method="post" action="view.php">
                    ...
                    <table>
                        ...
                        <tr>
                            ...
                            <td align="right"> 
                                <input type="button" name="go_btn" value="<?php print_string('save', 'promising') ?>" onclick="senddata('save')" />
                            </td>
                        </tr>
                    </table>
                    <input type="hidden" name="itemid" value="0" />
