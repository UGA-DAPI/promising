...
class Specification_Form extends moodleform {
	...
	function __construct($action, &$project, $mode, $specid){
		...
		$this->mode = $mode;
		if ($specid){
			$this->current = $DB->get_record('promising_requirement', array('id' => $specid));
		}
	...
	function definition(){
    	...
    	$mform->addElement('text', 'abstract', get_string('spectitle', 'promising'), array('size' => "100%"));

        $severities = promising_get_options('severity', $this->project->id);
        $severityoptions = array();
        foreach($severities as $aSeverity){
            $severityoptions[$aSeverity->code] = '['. $aSeverity->code . '] ' . $aSeverity->label;
        }
    	$mform->addElement('select', 'severity', get_string('severity', 'promising'), $severityoptions);
		$mform->addHelpButton('severity', 'severity', 'promising');

        $priorities = promising_get_options('priority', $this->project->id);
        $priorityoptions = array();
        foreach($priorities as $aPriority){
            $priorityoptions[$aPriority->code] = '['. $aPriority->code . '] ' . $aPriority->label;
        }
    	$mform->addElement('select', 'priority', get_string('priority', 'promising'), $priorityoptions);
		$mform->addHelpButton('priority', 'priority', 'promising');

        $complexities = promising_get_options('complexity', $this->project->id);
    	...
    	$mform->addElement('select', 'complexity', get_string('complexity', 'promising'), $complexityoptions);
		$mform->addHelpButton('complexity', 'complexity', 'promising');
		...
		if ($this->project->projectusesrequs && $this->mode == 'update'){
	        $requirements = promising_get_tree_options('promising_requirement', $this->project->id, $currentGroup);
	        $selection = $DB->get_records_select_menu('promising_spec_to_req', "specid = {$this->current->id}", array(), 'reqid, specid');
	        $reqs = array();
			...
			$select = &$mform->addElement('select', 'spectoreq', get_string('spectoreq', 'promising'), $reqs, array('size' => 8));
			$select->setMultiple(true);
			$mform->addHelpButton('spectoreq', 'spec_to_reqs', 'promising');
		}
    ...
    function set_data($defaults){
		...
		$defaults = file_prepare_standard_editor($defaults, 'description', $this->descriptionoptions, $context, 'mod_promising', 'specificationdescription', $defaults->id);
