...
class Message_Form extends moodleform {
	...
	function __construct($action, $mode, &$project, $messageid, $messageParent){
		...
		if ($messageid){
			$this->current = $DB->get_record('promising_messages', array('id' => $messageid));
		}
	...
	function definition(){
		...
		if($this->mode=='add' && $this->parent>0){//Si on ajoute une réponse (un id de parent existe)
			...
			ORDER BY 
				d.modified
			";
			$discussion = $DB->get_record('promising_messages', array('id' => $this->parent));
			if($discussion->message!=''){
				...
				$discussionDetails .= "<p class='messageintro header'>".get_string('ecritpar', 'promising')." <b>".$fullname."</b>, ".get_string('ecritle', 'promising')." ".userdate($discussion->modified, $dateType)."</p>\n";
			...
			if ($messages = $DB->get_records_sql($query)) {
				...
				//affichages des messages
					...
					$discussionDetails .= "<div class='messagebox'>\n";
					$discussionDetails .= "<p class='messageintro header'>".get_string('ecritpar', 'promising')." <b>".$fullname."</b>, ".get_string('ecritle', 'promising')." ".userdate($message->modified, $dateType)."</p>\n";
					$discussionDetails .= "<div class='messagecorp'>".$message->message."</div>\n";
					$discussionDetails .= "</div>\n";
				}
			}
			$discussionDetails .= "<p>".get_string('repondrediscu', 'promising')." :</p>";
			...
			$mform->addElement('text', 'abstract', get_string('messagetitle', 'promising'), array('size' => "100%"));
		}
		
		
    	$mform->addElement('editor', 'message_editor', get_string('message', 'promising'), null, $this->descriptionoptions);
    ...
    function set_data($defaults){
		...
		$currenttext = file_prepare_draft_area($draftid_editor, $context->id, 'mod_promising', 'message_editor', $defaults->id, array('subdirs' => true), $defaults->message);
		$defaults = file_prepare_standard_editor($defaults, 'message', $this->descriptionoptions, $context, 'mod_promising', 'message', $defaults->id);
