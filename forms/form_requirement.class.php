...
class Requirement_Form extends moodleform {
	...
	function __construct($action, &$project, $mode, $reqid){
		...
		if ($reqid){
			$this->current = $DB->get_record('promising_requirement', array('id' => $reqid));
		}
	...
	function definition(){
    	...
    	$mform->addElement('text', 'abstract', get_string('requirementtitle', 'promising'), array('size' => "100%"));

        $strengthes = promising_get_options('strength', $this->project->id);
        $strengthoptions = array();
        foreach($strengthes as $aStrength){
            $strengthoptions[$aStrength->code] = '['. $aStrength->code . '] ' . $aStrength->label;
        }
    	$mform->addElement('select', 'strength', get_string('strength', 'promising'), $strengthoptions);
		$mform->addHelpButton('strength', 'strength', 'promising');

        $heavynesses = promising_get_options('heavyness', $this->project->id);
        $heavynessoptions = array();
        foreach($heavynesses as $aHeavyness){
            $heavynessoptions[$aHeavyness->code] = '['. $aHeavyness->code . '] ' . $aHeavyness->label;
        }
    	$mform->addElement('select', 'heavyness', get_string('heavyness', 'promising'), $heavynessoptions);
		$mform->addHelpButton('heavyness', 'heavyness', 'promising');
		...
		if ($this->project->projectusesspecs && $this->mode == 'update'){
	        ...
	        $selection = $DB->get_records_select_menu('promising_spec_to_req', "reqid = {$this->current->id}", array(''), 'specid, reqid');
	        $reqs = array();
			...
			$select = &$mform->addElement('select', 'spectoreq', get_string('assignedspecs', 'promising'), $linkedspecs, array('size' => 8));
			$select->setMultiple(true);
			$mform->addHelpButton('spectoreq', 'spec_to_req', 'promising');
		}
    ...
    function set_data($defaults){
		...
		$defaults = file_prepare_standard_editor($defaults, 'description', $this->descriptionoptions, $context, 'mod_promising', 'requirementdescription', $defaults->id);
