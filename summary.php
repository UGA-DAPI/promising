...
/// counting tasks
        ...
        $tasks = $DB->get_records_select('promising_task', "projectid = $project->id AND groupid = $currentGroupId AND fatherid = 0 ", array($project->id, $currentGroupId), '', 'id,abstract');
        $leafTasks = array();
        $leafTaskList = '';
        if ($tasks){
...
/// counting deliverables
        ...
        $deliverables = $DB->get_records_select('promising_deliverable', "projectid = $project->id AND groupid = $currentGroupId AND fatherid = 0 ", array($project->id, $currentGroupId), '', 'id,abstract');
        $leafDelivs = array();
        $leafDelivList = '';
        if ($deliverables){
            foreach($deliverables as $aDeliverable){
                $leafDelivs = array_merge($leafDelivs, promising_count_leaves('promising_deliverable', $aDeliverable->id, true)) ;
            }
...
/********************************************* Start producing summary ***************************/
    ...
    <table width="80%">
        <tr valign="top">
            <th align="left" width="60%">
                <?php 
				echo "<img style='vertical-align:middle;margin-right:4px;' src='".$OUTPUT->pix_url('equipe', 'promising')."' alt='".get_string('equipe', 'promising')."' />Equipe du projet : ".$project->name;
                /*print_string('summaryforproject', 'promising');
                echo $OUTPUT->help_icon('leaves', 'promising', true); */
                ?>
             </th>
	...
	for ($i=0;$i<3;$i++){
		...
		if(!$rolempty){
			echo "<p class='summary-email'><a href='mailto:".implode(',',$mailtoUsers)."' title='Envoyer un email à tous'><img src='".$OUTPUT->pix_url('/new_mail', 'promising')."' alt=\"Envoyer un email à tous\" /></a>".$roleNom." :</p>";
