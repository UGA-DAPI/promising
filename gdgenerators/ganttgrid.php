...
if (@$wasIncluded == 0){
    ...
    $project = $DB->get_record('promising', array('id' => $projectid));
...
$milestones = $DB->get_records_select('promising_milestone', "projectid = ? AND groupid = ? AND deadlineenable = 1", array($project->id, $currentGroupId), 'deadline');
$milemarks = array();
if ($milestones){
...
*/
    ...
    $fontFile = $CFG->dirroot . "/mod/promising/fonts/arial.ttf";
    ...
    if (@$outputType == 'HEADING'){
        ...
        for($i = 0 ; $i < $daysInRange ; ){
            ...
            if ($range - $startRange > 40){
                imagestring($im, 2, 1 + $startRange, 3, mb_convert_encoding(get_string(strtolower(jdmonthname($jdStart + $i, 1)), 'promising'), 'auto', 'utf8'), $colors['quiteblack']);
            }
            $toggle = !$toggle;
