...
?>
    ...
    <?php
        if ($USER->editmode == 'on' && has_capability('mod/promising:changerequs', $context)) {
        	echo "<br/><a href='view.php?id={$cm->id}&amp;work=add&amp;fatherid=0'>".get_string('addrequ','promising')."</a>&nbsp;";
        }
    	promising_print_requirements($project, $currentGroupId, 0, $cm->id);
        if ($USER->editmode == 'on' && has_capability('mod/promising:changerequs', $context)) {
        	...
        	echo "<a href='view.php?id={$cm->id}&amp;work=add&amp;fatherid=0'>".get_string('addrequ','promising')."</a>&nbsp;";
