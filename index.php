...
/// Get context information
    ...
    add_to_log($course->id, 'promising', 'view all', "index.php?id=$course->id", "");
...
/// Get all the appropriate data
    ...
    if (! $projects = get_all_instances_in_course('promising', $course)) {
...
/// Print the list of instances (your module will probably extend this)
    ...
    $strprojectend = get_string('projectend', 'promising');
        ...
        if ($course->format == 'weeks' or $course->format == 'topics') {
            if (isteacher($course->id)) {
                ...
                if ($project->usemaxgrade) {
                    $grade = $DB->get_record_sql("SELECT MAX(grade) as grade FROM {promising_grades}
                            WHERE projectid = $project->id AND userid = $USER->id GROUP BY userid");
                } else {
                    $grade = $DB->get_record_sql("SELECT AVG(grade) as grade FROM {promising_grades}
                            WHERE projectid = $project->id AND userid = $USER->id GROUP BY userid");
                }
                if ($grade) {
