    ...
    if ($work == 'docopy'){
        ...
        */    
        function unitCopy($project, $from, $to, $what, $detail = false){
            ...
            foreach(array_keys($what) as $aDatatable){
                ...
                echo '<tr><td align="left">' . get_string('copying', 'promising') . ' ' . get_string("{$aDatatable}s", 'promising') . '...';
                $DB->delete_records("promising_$aDatatable", array('projectid' => $project->id, 'groupid' => $to));
                if($records = $DB->get_records_select("promising_$aDatatable", "projectid = $project->id AND groupid = $from")){
                    ...
                    foreach($records as $aRecord){
                        ...
                        $insertedid = $DB->insert_record("promising_$aDatatable", $aRecord);
                        $copied[$aDatatable][$id] = $insertedid;
                    }
                }
                echo '</td><td align="right"><span class="technicals">' . get_string('done', 'promising') . '</span></td></tr>';
            }
            return $copied;
        }
        /**
        ...
        */
        ...
        foreach($targets as $aTarget){
            ...
            echo '<tr><td align="left">' . get_string('fixingforeignkeys', 'promising') . '...</td><td align="right">';
            ...
            $DB->delete_records('promising_assessment', array('projectid' => $project->id, 'groupid' => $aTarget));
            echo '<span class="technicals">' . get_string('done', 'promising') . '</td></tr>';
        }
        echo '</table>';
        echo $OUTPUT->box_end();
    }
...
/// Setup project copy operations by defining source and destinations 
    ...
    if ($work == 'what'){
        ...
        if (in_array($from, $to)){
            $errormessage = get_string('cannotselfcopy', 'promising');
            $work = 'setup';
        } else {
    ?>
    <center>
    <?php 
    echo $OUTPUT->heading(get_string('copywhat', 'promising')); 
    $toArray = array();
    foreach ($to as $aTarget) $toArray[] = $groups[$aTarget]->name;
    if ($from){
        echo $OUTPUT->box($groups[$from]->name . ' &gt;&gt; ' . implode(',',$toArray), 'center');
    } else {
        echo $OUTPUT->box(get_string('groupless', 'promising') . ' &gt;&gt; ' . implode(',',$toArray), 'center');
    }
    ?>
    ...
    <table cellpadding="5">
    <tr valign="top">
        <td align="right"><b><?php print_string('what', 'promising') ?></b></td>
        <td align="left">
            <p><b><?php print_string('entities', 'promising') ?></b></p>
            <p><input type="checkbox" name="headings" value="1" checked="checked" /> <?php print_string('headings', 'promising'); ?>
            <?php
            if (@$project->projectusesrequs) echo "<br/><input type=\"checkbox\" name=\"requs\" value=\"1\" checked=\"checked\" onclick=\"formControl('requs')\" /> " . get_string('requirements', 'promising');
            if (@$project->projectusesspecs) echo "<br/><input type=\"checkbox\" name=\"specs\" value=\"1\" checked=\"checked\" onclick=\"formControl('specs')\" /> " . get_string('specifications', 'promising');
            ?>
            <br/><input type="checkbox" name="tasks" value="1" checked="checked" onclick="formControl('tasks')" /> <?php print_string('tasks', 'promising'); ?>
            <br/><input type="checkbox" name="miles" value="1" checked="checked"  onclick="formControl('miles')" /> <?php print_string('milestones', 'promising'); ?>
            <?php
            if (@$project->projectusesspecs) echo "<br/><input type=\"checkbox\" name=\"deliv\" value=\"1\" checked=\"checked\"  onclick=\"formControl('deliv')\" /> " . get_string('deliverables', 'promising');
            ?>
            </p>
            <p><b><?php print_string('crossentitiesmappings', 'promising') ?></b></p>
            <?php
            if (@$project->projectusesrequs && @$project->projectusesspecs) echo "<p><input type=\"checkbox\" name=\"spectoreq\" value=\"1\" checked=\"checked\" /> <span id=\"spectoreq_span\" class=\"\"> " . get_string('spec_to_req', 'promising') . '</span>';
            if (@$project->projectusesspecs) echo "<br/><input type=\"checkbox\" name=\"tasktospec\" value=\"1\" checked=\"checked\" /> <span id=\"tasktospec_span\" class=\"\"> ". get_string('task_to_spec', 'promising') . '</span>';
            ?>
            <br/><input type="checkbox" name="tasktotask" value="1" checked="checked" /> <span id="tasktotask_span" class=""><?php print_string('task_to_task', 'promising'); ?></span>
            <?php
            if (@$project->projectusesdelivs) echo "<br/><input type=\"checkbox\" name=\"tasktodeliv\" value=\"1\" checked=\"checked\" /> <span id=\"tasktodeliv_span\" class=\"\"> " . get_string('task_to_deliv', 'promising') . '</span>';
            ?>
            </p>
        </td>
    </tr>
...
/// Copy last confirmation form 
    ...
    <center>
    <?php 
    echo $OUTPUT->heading(get_string('copyconfirm', 'promising')); 
    echo $OUTPUT->box(get_string('copyadvice', 'promising'), 'center');
    ?>
    <script type="text/javascript">
    //<![CDATA[
    function senddata(){
    ...
    <input type="hidden" name="work" value="" />
    <input type="checkbox" name="detail" value="1" /> <?php print_string('givedetail', 'promising') ?>
...
/// Copy first setup form 
    ...
    <center>
    <?php 
        echo $OUTPUT->heading(get_string('copysetup', 'promising')); 
    ...
    <table width="80%">
    <tr valign="top">
        <td align="right"><b><?php print_string('from', 'promising') ?></b></td>
        <td align="left">
    <?php 
        $fromgroups = array();
        if (!empty($groups)){
        ...
        echo html_writer::select($fromgroups,  'from', 0 + get_current_group($course->id), get_string('groupless', 'promising'));
    ?>
        </td>
    </tr>
    <tr valign="top">
        <td align="right"><b><?php print_string('upto', 'promising') ?></b></td>
        <td align="left">
            <select name="to[]" multiple="multiple" size="6" style="width : 80%">
    <?php
        echo "<option value=\"0\">".get_string('groupless', 'promising')."</option>";
        if (!empty($groups)){
