...
*/
function promising_edition_enable_button($cm, $course, $project, $editmode){
    global $CFG, $USER;
    ...
    if ($editmode == 'on') {
        ...
        $str .= "<input type=\"submit\" value=\"" .  get_string('disableedit', 'promising') . "\" />";
        $str .= "</form>";
    } else {
        $str = "<form method=\"get\"  style=\"display : inline\" action=\"view.php\">";
        $str.= "<input type=\"hidden\" name=\"editmode\" value=\"on\" />";
        $str .= "<input type=\"hidden\" name=\"id\" value=\"{$cm->id}\" />";
        $str .= "<input type=\"submit\" value=\"" .  get_string('enableedit', 'promising') . "\" />";
        $str .= "</form>";
    }
    return $str;
}
...
*/
function promising_print_assignement_info($project, $return = false) {
    ...
    if (! $cm = get_coursemodule_from_instance('promising', $project->id, $course->id)) {
	...
	//$string = '<b>'.get_string('currentphase', 'promising').'</b>: '.promising_phase($project, '', $course).'<br />';
    $string = '';
    ...
    if (count($milestones)>0){
		...
		if(isset($nextMilestone->abstract)){
			$string .= '<b>'.get_string('currentmilestone', 'promising').'</b> : '.$nextMilestone->abstract." <br />";//étape en cours
			if($nextMilestone->deadlineenable==1){
				$string .= '<b>'.get_string('currentmilestoneendtime', 'promising').'</b> : '.userdate($nextMilestone->deadline)." <br />";//date fin de l'étape
			}
		}
    }
    ...
    foreach ($dates as $type => $date) {
        if ($date) {
            ...
            $string .= '<b>'.get_string($type, 'promising').'</b>: '.userdate($date)." <br />";//($strdifference)
        }
    }
    $str .= $string;
...
*/
function promising_phase($project, $style='') {
    ...
    if ($dated[$i]->id == 'projectstart'){
        return get_string('phasestart', 'promising');
    } elseif ($dated[$i]->id == 'projectend'){
        return get_string('phaseend', 'promising');
    } else {
...
*/
function promising_print_specifications($project, $group, $fatherid, $cmid){
	global $CFG, $USER, $DB, $OUTPUT;
	static $level = 0;
	static $startuplevelchecked = false;

	promising_check_startup_level('specification', $fatherid, $level, $startuplevelchecked);
	...
	$query = "
	    SELECT 
	        ...
	        c.collapsed
	    FROM 
	        {promising_specification} s
	    LEFT JOIN
	        {promising_collapse} c
	    ON
	        s.id = c.entryid AND
	...
	if ($specifications = $DB->get_records_sql($query)) {
		...
		foreach($specifications as $specification){
        	...
        	$level++;
		    promising_print_single_specification($specification, $project, $group, $cmid, count($specifications));
			...
			promising_print_specifications($project, $group, $specification->id, $cmid);
			echo "</div>";
        	$level--;
		    echo "</div>";
		}
		...
		if ($level == 0){
			...
			print_string('nospecifications', 'promising');
			echo $OUTPUT->box_end();
		}
	}
}
...
*/
function promising_print_single_specification($specification, $project, $group, $cmid, $setSize, $fullsingle = false){
    global $CFG, $USER, $SESSION, $DB, $OUTPUT;

    $context = context_module::instance($cmid);
    $canedit = $USER->editmode == 'on' && has_capability('mod/promising:changespecs', $context);
    $numspec = implode('.', promising_tree_get_upper_branch('promising_specification', $specification->id, true, true));
    if (!$fullsingle){
    	if (promising_count_subs('promising_specification', $specification->id) > 0) {
			$ajax = $CFG->enableajax && $CFG->enablecourseajax;
    		$hidesub = "<a href=\"javascript:toggle('{$specification->id}','sub{$specification->id}', $ajax, '$CFG->wwwroot');\"><img name=\"img{$specification->id}\" src=\"{$CFG->wwwroot}/mod/promising/pix/p/switch_minus.gif\" alt=\"collapse\" /></a>";
    	} else {
    		$hidesub = "<img src=\"{$CFG->wwwroot}/mod/promising/pix/p/empty.gif\" />";
    	}
    } else {
       $hidesub = '';
    }
	...
	$specList = str_replace(",", "','", promising_get_subtree_list('promising_specification', $specification->id));
	$taskcount = promising_print_entitycount('promising_specification', 'promising_task_to_spec', $project->id, $group, 'spec', 'task', $specification->id, $specList);
	$reqcount = promising_print_entitycount('promising_specification', 'promising_spec_to_req', $project->id, $group, 'spec', 'req', $specification->id, $specList);
	...
	$completion = ($res->total != 0) ? promising_bar_graph_over($res->completion / $res->total, 0) : promising_bar_graph_over(-1, 0);
	$checkbox = ($canedit)? "<input type=\"checkbox\" id=\"sel{$specification->id}\" name=\"ids[]\" value=\"{$specification->id}\" /> " : '' ;
	$priorityoption = promising_get_option_by_key('priority', $project->id, $specification->priority);
	$severityoption = promising_get_option_by_key('severity', $project->id, $specification->severity);
	$complexityoption = promising_get_option_by_key('complexity', $project->id, $specification->complexity);
    /*
	...
	*/
	$prioritysignal = "<img src=\"".$OUTPUT->pix_url("/priority_{$priorityoption->truelabel}", 'promising')."\" title=\"{$priorityoption->label}\" />";
	$severitysignal = "<img src=\"".$OUTPUT->pix_url("/severity_{$severityoption->truelabel}", 'promising')."\" title=\"{$severityoption->label}\" />";
	$complexitysignal = "<img src=\"".$OUTPUT->pix_url("/complexity_{$complexityoption->truelabel}", 'promising')."\" title=\"{$complexityoption->label}\" />";

	if (!$fullsingle){
	    $hideicon = (!empty($specification->description)) ? 'hide' : 'hide_shadow' ;
	    $hidedesc = "<a href=\"javascript:toggle_show('{$numspec}','{$numspec}', '$CFG->wwwroot');\"><img name=\"eye{$numspec}\" src=\"".$OUTPUT->pix_url("/p/{$hideicon}", 'promising')."\" alt=\"collapse\" /></a>";
	} else {
	    $hidedesc = '';
	}
    ...
    $innertable->data[] = array(get_string('priority','promising'), "<span class=\"scale{$priorityoption->id}\" title=\"{$priorityoption->label}\">{$priorityoption->label}</span>");
    $innertable->data[] = array(get_string('severity','promising'), "<span class=\"scale{$severityoption->id}\" title=\"{$severityoption->label}\">{$severityoption->label}</span>");
    $parms = promising_print_project_table($innertable, true);
    $description = file_rewrite_pluginfile_urls($specification->description, 'pluginfile.php', $context->id, 'mod_promising', 'specificationdescription', $specification->id);
    ...
    if (!$fullsingle){
	    $desc .= "<br/><a href=\"{$CFG->wwwroot}/mod/promising/view.php?id={$cmid}&amp;view=view_detail&amp;objectId={$specification->id}&amp;objectClass=specification\" >".get_string('seedetail','promising')."</a></p>"; 
    }
    $desc .= '</div>';
	...
	if ($canedit) {
		...
		$link[] = "<a href='view.php?id={$cmid}&amp;work=add&amp;fatherid={$specification->id}&amp;view=specifications'>
				 <img src='".$OUTPUT->pix_url('/p/newnode', 'promising')."' alt=\"".get_string('addsubspec', 'promising')."\" /></a>";
		...
		$templateicon = ($specification->id == @$SESSION->promising->spectemplateid) ? "{$CFG->wwwroot}/mod/promising/pix/p/activetemplate.gif" : "{$CFG->wwwroot}/mod/promising/pix/p/marktemplate.gif" ;
		$link[] = "<a href='view.php?id={$cmid}&amp;work=domarkastemplate&amp;specid={$specification->id}&amp;view=specifications#node{$specification->id}'>
				 <img src='$templateicon' title=\"".get_string('markastemplate', 'promising')."\" /></a>";
		if ($specification->ordering > 1)
		    $link[] = "<a href='view.php?id={$cmid}&amp;work=up&amp;specid={$specification->id}&amp;view=specifications#node{$specification->id}'>
				 <img src='".$OUTPUT->pix_url('/t/up')."' title=\"".get_string('up', 'promising')."\" /></a>";
		if ($specification->ordering < $setSize)
		    $link[] = "<a href='view.php?id={$cmid}&amp;work=down&amp;specid={$specification->id}&amp;view=specifications#node{$specification->id}'>
				 <img src='".$OUTPUT->pix_url('/t/down')."' title=\"".get_string('down', 'promising')."\" /></a>";
	    if ($specification->fatherid != 0)
		    $link[] = "<a href='view.php?id={$cmid}&amp;work=left&amp;specid={$specification->id}&amp;view=specifications#node{$specification->id}'>
				 <img src='".$OUTPUT->pix_url('/t/left')."' title=\"".get_string('left', 'promising')."\" /></a>";
		if ($specification->ordering > 1)
		    $link[] = "<a href='view.php?id={$cmid}&amp;work=right&amp;specid={$specification->id}&amp;view=specifications#node{$specification->id}'>
				 <img src='".$OUTPUT->pix_url('/t/right')."' title=\"".get_string('right', 'promising')."\" /></a>";
	...
	promising_print_project_table($table);
	unset($table);
}
...
* @uses $USER
*/
function promising_print_requirements($project, $group, $fatherid, $cmid){
	global $CFG, $USER, $DB, $OUTPUT;
	static $level = 0;
	static $startuplevelchecked = false;

	promising_check_startup_level('requirement', $fatherid, $level, $startuplevelchecked);
	...
	$query = "
	    SELECT 
	        ...
	        c.collapsed
	    FROM 
	        {promising_requirement} r
	    LEFT JOIN
	        {promising_spec_to_req} str
	    ON
	        r.id = str.reqid
        LEFT JOIN
            {promising_collapse} c
	...
	if ($requirements = $DB->get_records_sql($query)) {
		...
		foreach($requirements as $requirement){
		    echo "<div class=\"nodelevel{$level}\">";
		    $level++;
		    promising_print_single_requirement($requirement, $project, $group, $cmid, count($requirements));
			...
			promising_print_requirements($project, $group, $requirement->id, $cmid);
			echo "</div>";
		    $level--;
			echo "</div>";
		}
		...
		if ($level == 0){
			echo $OUTPUT->box_start();
			print_string('norequirements', 'promising');
			echo $OUTPUT->box_end();
		}
	}
}
...
* @param fullsingle true if prints a single isolated element
*/
function promising_print_single_requirement($requirement, $project, $group, $cmid, $setSize, $fullsingle = false){
    global $CFG, $USER, $DB, $OUTPUT;

    $context = context_module::instance($cmid);
    $canedit = $USER->editmode == 'on' && has_capability('mod/promising:changerequs', $context);
    $numrequ = implode('.', promising_tree_get_upper_branch('promising_requirement', $requirement->id, true, true));
    if (!$fullsingle){
    	if (promising_count_subs('promising_requirement', $requirement->id) > 0) {
    		$ajax = $CFG->enableajax && $CFG->enablecourseajax;
    		$hidesub = "<a href=\"javascript:toggle('{$requirement->id}','sub{$requirement->id}', '$ajax', '$CFG->wwwroot');\"><img name=\"img{$requirement->id}\" src=\"".$OUTPUT->pix_url('/p/switch_minus', 'promising')."\" alt=\"collapse\" /></a>";
    	} else {
    		$hidesub = "<img src=\"".$OUTPUT->pix_url('/p/empty', 'promising')."\" />";
    	}
    } else {
       $hidesub = '';
    }
	...
	$reqList = str_replace(",", "','", promising_get_subtree_list('promising_requirement', $requirement->id));
	$speccount = promising_print_entitycount('promising_requirement', 'promising_spec_to_req', $project->id, $group, 'req', 'spec', $requirement->id, $reqList);
	$checkbox = ($canedit) ? "<input type=\"checkbox\" id=\"sel{$requirement->id}\" name=\"ids[]\" value=\"{$requirement->id}\" /> " : '';

	$strengthoption = promising_get_option_by_key('strength', $project->id, $requirement->strength);
    $strengthsignal = '';
    if (!empty($requirement->strength)){
	    $strengthsignal = "<span class=\"scale_{$strengthoption->truelabel}\" title=\"{$strengthoption->label}\">s</span>";
    }

	$heavynessoption = promising_get_option_by_key('heaviness', $project->id, $requirement->heavyness);
    $heavynessclass = '';
	...
	if (!$fullsingle){
	    ...
	    $hidedesc = "<a href=\"javascript:toggle_show('{$numrequ}','{$numrequ}', '$CFG->wwwroot');\"><img name=\"eye{$numrequ}\" src=\"".$OUTPUT->pix_url("/p/{$hideicon}", 'promising')."\" alt=\"collapse\" /></a>";
	} else {
	    $hidedesc = '';
	}
    ...
    $innertable->style = array('parmname', 'parmvalue');
	$innertable->align = array ('left', 'left');
    $innertable->data[] = array(get_string('strength', 'promising'), "<span class=\"scale{$strengthoption->id}\" title=\"{$strengthoption->label}\">{$strengthoption->label}</span>");
    $parms = promising_print_project_table($innertable, true);
    $description = file_rewrite_pluginfile_urls($requirement->description, 'pluginfile.php', $context->id, 'mod_promising', 'requirementdescription', $requirement->id);
    ...
    if (!$fullsingle){
	    $desc .= "<br/><a href=\"{$CFG->wwwroot}/mod/promising/view.php?id={$cmid}&amp;view=view_detail&amp;objectId={$requirement->id}&amp;objectClass=requirement\" >".get_string('seedetail','promising')."</a>"; 
    }
    $desc .='</div>';
	...
	if ($canedit) {
		...
		$link[] = "<a href='view.php?id={$cmid}&amp;work=add&amp;fatherid={$requirement->id}&amp;view=requirements'>
				 <img src='".$OUTPUT->pix_url('/p/newnode', 'promising')."' alt=\"".get_string('addsubrequ', 'promising')."\" /></a>";
		...
		if ($requirement->ordering > 1)
		    $link[] = "<a href='view.php?id={$cmid}&amp;work=up&amp;requid={$requirement->id}&amp;view=requirements#node{$requirement->id}'>
				 <img src='".$OUTPUT->pix_url('/t/up')."' alt=\"".get_string('up', 'promising')."\" /></a>";
		if ($requirement->ordering < $setSize)
		    $link[] = "<a href='view.php?id={$cmid}&amp;work=down&amp;requid={$requirement->id}&amp;view=requirements#node{$requirement->id}'>
				 <img src='".$OUTPUT->pix_url('/t/down')."' alt=\"".get_string('down', 'promising')."\" /></a>";
	    if ($requirement->fatherid != 0)
		    $link[] = "<a href='view.php?id={$cmid}&amp;work=left&amp;requid={$requirement->id}&amp;view=requirements#node{$requirement->id}'>
				 <img src='".$OUTPUT->pix_url('/t/left')."' alt=\"".get_string('left', 'promising')."\" /></a>";
		if ($requirement->ordering > 1)
		    $link[] = "<a href='view.php?id={$cmid}&amp;work=right&amp;requid={$requirement->id}&amp;view=requirements#node{$requirement->id}'>
				 <img src='".$OUTPUT->pix_url('/t/right')."' alt=\"".get_string('right', 'promising')."\" /></a>";
		$table->data[] = array($indent . implode(' ', $link));
	    $table->rowclass[] = 'controls';
	}
...
* @param cmid the module id (for urls)
*/
function promising_print_tasks($project, $group, $fatherid, $cmid, $propagated=null){
	global $CFG, $USER, $DB, $OUTPUT;
	static $level = 0;
	static $startuplevelchecked = false;

	promising_check_startup_level('task', $fatherid, $level, $startuplevelchecked);
	...
	if ($tasks = $DB->get_records_sql($query)) {
		foreach($tasks as $task){
		    ...
		    if (!@$propagated->collapsed || !$CFG->enablecourseajax || !$CFG->enableajax){
			    promising_print_single_task($task, $project, $group, $cmid, count($tasks), false, '');
			}
			...
			promising_print_tasks($project, $group, $task->id, $cmid, $propagatedroot);
			echo "</div>";
			$level--;
			echo "</div>";
		}
		...
		if ($level == 0){
			echo $OUTPUT->box_start();
			print_string('notasks', 'promising');
			echo $OUTPUT->box_end();
		}
	}
}
...
*/
function promising_print_single_task($task, $project, $group, $cmid, $setSize, $fullsingle = false, $style='', $nocollapse = false){
    global $CFG, $USER, $SESSION, $DB, $OUTPUT;
    ...
    $context = context_module::instance($cmid);
    $canedit = ($USER->editmode == 'on') && has_capability('mod/promising:changetasks', $context) && !preg_match("/NOEDIT/", $style);
    if (!has_capability('mod/promising:changenotownedtasks', $context)){
        if ($task->owner != $USER->id) $canedit = false;
    }
	$hasMasters = $DB->count_records('promising_task_dependency', array('slave' => $task->id));
	$hasSlaves = $DB->count_records('promising_task_dependency', array('master' => $task->id));
	$taskDependency = "<img src=\"{$CFG->wwwroot}/mod/promising/pix/p/task_alone.gif\" title=\"".get_string('taskalone','promising')."\" />";
	if ($hasSlaves && $hasMasters){
	    $taskDependency = "<img src=\"".$OUTPUT->pix_url('/p/task_middle', 'promising')."\" title=\"".get_string('taskmiddle','promising')."\" />";
	} else if ($hasMasters){
	    $taskDependency = "<img src=\"".$OUTPUT->pix_url('/p/task_end', 'promising')."\" title=\"".get_string('taskend','promising')."\" />";
	} else if ($hasSlaves){
	    $taskDependency = "<img src=\"".$OUTPUT->pix_url('/p/task_start', 'promising')."\" title=\"".get_string('taskstart','promising')."\" />";
	}

    $numtask = implode('.', promising_tree_get_upper_branch('promising_task', $task->id, true, true));
    if (!$fullsingle && !$nocollapse){
    	if (promising_count_subs('promising_task', $task->id) > 0) {
    		$ajax = $CFG->enableajax && $CFG->enablecourseajax;
    		$hidesub = "<a href=\"javascript:toggle('{$task->id}','sub{$task->id}', '$ajax', '$CFG->wwwroot');\"><img name=\"img{$task->id}\" src=\"".$OUTPUT->pix_url('/p/switch_minus', 'promising')."\" alt=\"collapse\" /></a>";
    	} else {
    		$hidesub = "<img src=\"".$OUTPUT->pix_url('/p/empty', 'promising')."\" />";
    	}
    } else {
	...
	$taskcount = promising_print_entitycount('promising_task', 'promising_task_to_spec', $project->id, $group, 'task', 'spec', $task->id);
	$delivcount = promising_print_entitycount('promising_task', 'promising_task_to_deliv', $project->id, $group, 'task', 'deliv', $task->id);
    ...
    $completion = promising_bar_graph_over($task->done, $over, $barwidth, 5);
    $milestonepix = (isset($task->milestoneforced)) ? 'milestoneforced' : 'milestone' ; 
    $milestone = ($task->milestoneid) ? "<img src=\"".$OUTPUT->pix_url("/p/{$milestonepix}", 'promising')."\" title=\"".format_string(@$task->milestoneabstract)."\" />" : '';
	if (!$fullsingle || $fullsingle === 'HEAD'){
	    $hideicon = (!empty($task->description)) ? 'hide' : 'hide_shadow' ;
	    $hidetask = "<a href=\"javascript:toggle_show('{$numtask}','{$numtask}', '{$CFG->wwwroot}');\"><img name=\"eye{$numtask}\" src=\"".$OUTPUT->pix_url("/p/{$hideicon}", 'promising')."\" alt=\"collapse\" /></a>";
	} else {
	    $hidetask = '';
	}
	$assigneestr = '';
	...
	if (!preg_match('/SHORT_WITHOUT_ASSIGNEE/', $style) && $task->assignee){
	   ...
	   $headdetaillink = "<a href=\"{$CFG->wwwroot}/mod/promising/view.php?id={$cmid}&amp;view=view_detail&amp;objectId={$task->id}&amp;objectClass=task\" ><img src=\"{$CFG->wwwroot}/mod/promising/pix/p/hide.gif\" title=\"".get_string('detail', 'promising')."\" /></a>";
	}

    $worktypeicon = '';
    $worktypeoption = promising_get_option_by_key('worktype', $project->id, $task->worktype);
    if ($style == '' || !$style === 'SHORT_WITHOUT_TYPE'){
    	if (file_exists("{$CFG->dirroot}/mod/promising/pix/p/".@$worktypeoption->code.".gif")){
    	    $worktypeicon = "<img src=\"".$OUTPUT->pix_url("/p/{$worktypeoption->code}", 'promising')."\" title=\"".$worktypeoption->label."\" height=\"24\" align=\"middle\" />";
    	}
    }
    $orderCell = '';
    if (preg_match('/SHORT_WITH_ASSIGNEE_ORDERED/', $style)){
        ...
        if (!isset($order)) 
            ...
            $order++;
        $priorityDesc = promising_get_option_by_key('priority', $project->id, $task->priority);
	...
	$statusoption = promising_get_option_by_key('taskstatus', $project->id, $task->status);
    ...
    $innertable->style = array('parmname', 'parmvalue');
	$innertable->align = array ('left', 'left');
    $innertable->data[] = array(get_string('worktype','promising'), $worktypeoption->label);
    $innertable->data[] = array(get_string('status','promising'), $statusoption->label);
    $innertable->data[] = array(get_string('costrate','promising'), $task->costrate);
    ...
    $innertable->data[] = array(get_string('costplanned','promising'), $planned);
    $quote = $task->quoted . ' ' . $project->costunit;
    if ($project->useriskcorrection){
        $quote .= "<span class=\"riskshifted\">(".($task->quoted * (1 + ($task->risk / 100))) . ' ' . $project->costunit.")</span>";
    }
    $innertable->data[] = array(get_string('quoted','promising'), $quote);
    $innertable->data[] = array(get_string('risk','promising'), $task->risk);
    $innertable->data[] = array(get_string('done','promising'), $task->done . '%');
    $innertable->data[] = array(get_string('used','promising'), $task->used . ' ' . $TIMEUNITS[$project->timeunit]);
    $innertable->data[] = array(get_string('spent','promising'), $task->spent . ' ' . $project->costunit);
    $innertable->data[] = array(get_string('mastertasks','promising'), $hasMasters);
    $innertable->data[] = array(get_string('slavetasks','promising'), $hasSlaves);
    $parms = promising_print_project_table($innertable, true);
    $description = file_rewrite_pluginfile_urls($task->description, 'pluginfile.php', $context->id, 'mod_promising', 'taskdescription', $task->id);
    ...
    if (!$fullsingle || $fullsingle === 'SHORT' || $fullsingle === 'SHORT_WITHOUT_TYPE'){
	    $desc .= "<br/><a href=\"{$CFG->wwwroot}/mod/promising/view.php?id={$cmid}&amp;view=view_detail&amp;objectId={$task->id}&amp;objectClass=task\" >".get_string('seedetail','promising')."</a></p>"; 
    }
    $desc .= "</div>";
	...
	if ($canedit) {
		...
		$link[] = "<a href='view.php?id={$cmid}&amp;work=add&amp;fatherid={$task->id}&amp;view=tasks'>
				 <img src='".$OUTPUT->pix_url('/p/newnode', 'promising')."' title=\"".get_string('addsubtask', 'promising')."\" /></a>";
		$link[] = "<a href='view.php?id={$cmid}&amp;work=update&amp;taskid={$task->id}&amp;view=tasks'>
				 <img src='".$OUTPUT->pix_url('/t/edit')."' title=\"".get_string('updatetask', 'promising')."\" /></a>";
		$templateicon = ($task->id == @$SESSION->promising->tasktemplateid) ? $OUTPUT->pix_url('/p/activetemplate', 'promising') : $OUTPUT->pix_url('/p/marktemplate', 'promising') ;
		...
		$link[] = "<a href='view.php?id={$cmid}&amp;work=delete&amp;taskid={$task->id}&amp;view=tasks'>
				 <img src='".$OUTPUT->pix_url('/t/delete')."' title=\"".get_string('deletetask', 'promising')."\" /></a>";
		if ($task->ordering > 1)
...
* @param cmid the module id (for urls)
*/
function promising_print_milestones($project, $group, $numstage, $cmid){
	global $CFG, $USER, $DB, $OUTPUT;
	
    $TIMEUNITS = array(get_string('unset','promising'),get_string('hours','promising'),get_string('halfdays','promising'),get_string('days','promising'));
    $context = context_module::instance($cmid);
    $canedit = $USER->editmode == 'on' && has_capability('mod/promising:changemiles', $context);
	//bug editmode a resoudre avant de l'ajouter...
	//$canvalidatemilestone = $USER->editmode == 'on' && has_capability('mod/promising:validatemilestone', $context);
	//$canaskvalidatemilestone = $USER->editmode == 'on' && has_capability('mod/promising:askvalidatemilestone', $context);
	$canvalidatemilestone =  has_capability('mod/promising:validatemilestone', $context);
	$canaskvalidatemilestone = has_capability('mod/promising:askvalidatemilestone', $context);
	
	if ($milestones = $DB->get_records_select('promising_milestone', "projectid = ? AND groupid = ? ", array($project->id, $group),'ordering ASC' )) {
		$i = 1;
		...
		foreach($milestones as $milestone){		
			...
			$deliverables = $DB->get_records('promising_deliverable', array('milestoneid' => $milestone->id, 'projectid' => $project->id,'typeelm' => 1), '', 'abstract,id');
			$ressources = $DB->get_records('promising_deliverable', array('milestoneid' => $milestone->id, 'projectid' => $project->id,'typeelm' => 0), '', 'abstract,id');
			//$taskcount = "<img src=\"".$OUTPUT->pix_url('/p/task', 'promising')."\" />[".$taskCount."]";
			//$deliverablecount = " <img src=\"".$OUTPUT->pix_url('/p/deliv', 'promising')."\" />[".$delivCount."]";
			echo "<tr class='milestonehead'>";
			echo "<td><img style='vertical-align:middle;margin-right:3px;' src=\"{$CFG->wwwroot}/mod/promising/pix/etape_16x16.png\" /><span class='step-title'>Etape {$i} - ".format_string($milestone->abstract)."</span></td>";
			...
			// $hide = "<a href=\"javascript:toggle_show('{$i}','{$i}');\"><img name=\"eye{$i}\" src=\"".$OUTPUT->pix_url('/p/hide', 'promising')."\" alt=\"collapse\" /></a>";
			/*
			$hide = '';
			...
			*/
			...
			if ($canedit) {
				...
				if ($i > 1)
				    $link[] = "<a title='".get_string('up', 'promising')."' href='view.php?id={$cmid}&amp;work=up&amp;milestoneid={$milestone->id}&amp;view=milestones'>
						 <img src='".$OUTPUT->pix_url('/t/up')."' alt=\"".get_string('up', 'promising')."\" /></a>";
				if ($i < count($milestones))
				    $link[] = "<a title='".get_string('down', 'promising')."' href='view.php?id={$cmid}&amp;work=down&amp;milestoneid={$milestone->id}&amp;view=milestones'>
						 <img src='".$OUTPUT->pix_url('/t/down')."' alt=\"".get_string('down', 'promising')."\" /></a>";
				//$table->data[] = array(implode(' ', $link));
				$controls = implode(' ', $link);
			}
			//var_dump(has_capability('mod/promising:validatemilestone', $context));
			//var_dump(has_capability('mod/promising:askvalidatemilestone', $context));
			$query = "
			   SELECT
				   count(*) as count
			   FROM
			...
			if($project->typeprojet>0){//si c'est un projet on affiche les actions possible
				if ($canvalidatemilestone && $milestone->statut == 1) {//Si on peut valider une étape et que le statut de l'étape est demande de validation
					$actionsEtape = "<p onclick=\"javascript:var r=confirm('Etes vous sure de vouloir valider l\'étape ".addslashes($milestone->abstract)." ?');if (r==true){window.location.href='view.php?id={$cmid}&amp;work=valider&amp;milestoneid={$milestone->id}&amp;view=milestones';}\" class='btn-action'>
							<img src='".$OUTPUT->pix_url('/valide32', 'promising')."' alt=\"".get_string('VALIDERMILESTONE', 'promising')."\" /></p>";
					$actionsEtape .= "<p onclick=\"javascript:var r=confirm('Etes vous sure de vouloir mettre en révision l\'étape ".addslashes($milestone->abstract)." ?');if (r==true){window.location.href='view.php?id={$cmid}&amp;work=refuser&amp;milestoneid={$milestone->id}&amp;view=milestones';}\" class='btn-action'>
							<img src='".$OUTPUT->pix_url('/refu32', 'promising')."' alt=\"".get_string('REFUSERMILESTONE', 'promising')."\" /></p>";
						...
						$actionsEtape = "<p onclick=\"javascript:var r=confirm('Etes vous sure de vouloir demander la validation de l\'étape ".addslashes($milestone->abstract)." ?');if (r==true){window.location.href='view.php?id={$cmid}&amp;work=askvalider&amp;milestoneid={$milestone->id}&amp;view=milestones';}\" class='btn-action'>
							<img src='".$OUTPUT->pix_url('/askvalide32', 'promising')."' alt=\"".get_string('ASKVALIDERMILESTONE', 'promising')."\" /></p>";
				}
				/* si on veut un picto sur en cours de validation vu des étudiants
				elseif($milestone->statut== 1){
						$actionsEtape = "<img src='".$OUTPUT->pix_url('/waitvalide32', 'promising')."' alt=\"".get_string('WAITVALIDERMILESTONE', 'promising')."\" />";
				}*/
			...
			if(count($ressources)>0){
				foreach($ressources as $ressource){
					...
					$files = $fs->get_area_files($context->id, 'mod_promising', 'deliverablelocalfile', $ressource->id, 'sortorder DESC, id ASC', false);
					if(!empty($files)){
						$file = reset($files);
						$path = '/'.$context->id.'/mod_promising/deliverablelocalfile/'.$file->get_itemid().$file->get_filepath().$file->get_filename();
			...
			$fs = get_file_storage();
			$files = $fs->get_area_files($context->id, 'mod_promising', 'deliverablearchive', $milestone->id, 'sortorder DESC, id ASC', false);
			if(!empty($files)){
				$j=0;
				foreach($files as $file){
					...
					$path = '/'.$context->id.'/mod_promising/deliverablearchive/'.$file->get_itemid().$file->get_filepath().$file->get_filename();
					$url = moodle_url::make_file_url('/pluginfile.php', $path, '');
		...
		echo $OUTPUT->box_start();
		print_string('nomilestones', 'promising');
		echo $OUTPUT->box_end();
	}
}
...
* @uses $USER
*/
function promising_print_deliverables($project, $group, $fatherid, $cmid, $propagated=null){
	global $CFG, $USER, $DB, $OUTPUT;
	static $level = 0;
	static $startuplevelchecked = false;

	promising_check_startup_level('deliverable', $fatherid, $level, $startuplevelchecked);	
	
	echo "<div class='sepbloc'></div>";
	promising_print_bloc_deliverables($project, $group,$fatherid, $cmid,$propagated, 1);
	promising_print_bloc_deliverables($project, $group,$fatherid, $cmid,$propagated, 0);
	
	echo "<div class='sepbloc'></div>";
}


function promising_print_bloc_deliverables($project, $group,$fatherid, $cmid,$propagated, $typeelm){
	global $CFG, $USER, $DB, $OUTPUT;
	static $level = 0;
	static $startuplevelchecked = false;
	if($typeelm==1){
	...
	$query = "
	    ...
	    ORDER BY 
	        d.ordering
	";
	echo "<div class='".$classBloc."'><h3 class='header'><img src='".$OUTPUT->pix_url($lblBloc, 'promising')."' alt='".get_string($lblBloc, 'promising')."' />".get_string($lblBloc, 'promising')."</h3>";
	if ($deliverables = $DB->get_records_sql($query)) {
		foreach($deliverables as $deliverable){
            ...
            promising_print_single_deliverable($deliverable, $project, $group, $cmid, count($deliverables));

            $visibility = ($deliverable->collapsed) ? 'display: none' : 'display: block' ; 
			echo "<div id=\"sub{$deliverable->id}\" style=\"$visibility\" >";
			//promising_print_deliverables($project, $group, $deliverable->id, $cmid, $propagatedroot);
			echo "</div>";
			$level--;
			echo "</div>";
		}
		...
		if ($level == 0){
			echo $OUTPUT->box_start();
			if($typeelm==1){
				print_string('nodeliverables', 'promising');
			}else{
				print_string('noressources', 'promising');
			}
			echo $OUTPUT->box_end();
		}
	}
...
* @param fullsingle true if prints a single isolated element
*/
function promising_print_single_deliverable($deliverable, $project, $group, $cmid, $setSize, $fullsingle = false){
    global $CFG, $USER, $DB, $OUTPUT;

    $context = context_module::instance($cmid);
    $canedit = $USER->editmode == 'on' && has_capability('mod/promising:changedelivs', $context);
    $numdeliv = implode('.', promising_tree_get_upper_branch('promising_deliverable', $deliverable->id, true, true));
	/*
	if (!$fullsingle){
    	if (promising_count_subs('promising_deliverable', $deliverable->id) > 0) {
    		$ajax = $CFG->enableajax && $CFG->enablecourseajax;
    		$hidesub = "<a href=\"javascript:toggle('{$deliverable->id}','sub{$deliverable->id}', '$ajax', '$CFG->wwwroot');\"><img name=\"img{$deliverable->id}\" src=\"{$CFG->wwwroot}/mod/promising/pix/p/switch_minus.gif\" alt=\"collapse\" /></a>";
    	} else {
    		$hidesub = "<img src=\"{$CFG->wwwroot}/mod/promising/pix/p/empty.gif\" />";
    	}
    }
	...
	*/
	...
	$query = "
       ...
       FROM
          {promising_task} as t,
          {promising_task_to_deliv} as ttd
       WHERE
          t.projectid = {$project->id} AND
          t.groupid = $group AND
          t.id = ttd.taskid AND
    ...
    if ($res = $DB->get_record_sql($query)){ 
         if ($res->count != 0){
            ...
            $completion = promising_bar_graph_over($deliverable->done, $over, 60, 5);
        }
    }

	$completion = '';
    $milestonepix = (isset($deliverable->milestoneforced)) ? 'milestoneforced' : 'milestone' ; 
    $milestone = ($deliverable->milestoneid) ? "<img src=\"".$OUTPUT->pix_url("/p/{$milestonepix}", 'promising')."\" title=\"".@$deliverable->milestoneabstract."\" />" : '';
	*/
	$milestone ='';
	$taskcount = promising_print_entitycount('promising_deliverable', 'promising_task_to_deliv', $project->id, $group, 'deliv', 'task', $deliverable->id);
	...
	if (!$fullsingle){
	    ...
	    //$hidedeliv = "<a href=\"javascript:toggle_show('{$numdeliv}','{$numdeliv}', '$CFG->wwwroot');\"><img name=\"eye{$numdeliv}\" src=\"".$OUTPUT->pix_url("/p/{$hideicon}", 'promising')."\" alt=\"collapse\" /></a>";
		 $hidedeliv = '';
	} else {
	    $hidedeliv = '';
	}
	...
	if ($deliverable->localfile){
		...
		$files = $fs->get_area_files($context->id, 'mod_promising', 'deliverablelocalfile', $deliverable->id, '', true);
		if(count($files)>0){
			$file = reset($files);
			//$url = moodle_url::make_pluginfile_url($context->id, 'promising', 'introimg', $project->id, '', $imgFileName);
			$path = '/'.$context->id.'/mod_promising/deliverablelocalfile/'.$file->get_itemid().$file->get_filepath().$file->get_filename();
			$url = moodle_url::make_file_url('/pluginfile.php', $path, '');
			
			//$localfile = "{$project->course}/moddata/promising/{$project->id}/".md5("promising{$project->id}_{$group}")."/{$deliverable->localfile}";
...
/*
	$statusoption = promising_get_option_by_key('delivstatus', $project->id, $deliverable->status);
	...
	$innertable->align = array ('left', 'left');
    //$innertable->data[] = array(get_string('status','promising'), $statusoption->label);
    //$innertable->data[] = array(get_string('fromtasks','promising'), $taskcount);
    $parms = promising_print_project_table($innertable, true);
    $description = file_rewrite_pluginfile_urls($deliverable->description, 'pluginfile.php', $context->id, 'mod_promising', 'deliverabledescription', $deliverable->id);
    ...
    if (!$fullsingle){
	    //$desc .= "<br/><a href=\"{$CFG->wwwroot}/mod/promising/view.php?id={$cmid}&amp;view=view_detail&amp;objectId={$deliverable->id}&amp;objectClass=deliverable\" >".get_string('seedetail','promising')."</a></div>"; 
    }
    $desc .= "</div>";
	...
	*/
	if ($canedit) {
		...
		/*$link[] = "<a href=\"view.php?id={$cmid}&amp;work=add&amp;fatherid={$deliverable->id}&amp;view=deliverables\">
				 <img src=\"".$OUTPUT->pix_url('/p/newnode', 'promising')."\" alt=\"".get_string('addsubdeliv', 'promising')."\" /></a>";*/
...
*/
function promising_print_heading(&$project, $group){
    global $CFG, $DB, $OUTPUT;
    
    $projectheading = $DB->get_record('promising_heading', array('projectid' => $project->id, 'groupid' => $group));
    ...
    if (!$projectheading){
        ...
        $DB->insert_record('promising_heading', $projectheading);
    }
    //echo $OUTPUT->heading(get_string('projectis', 'promising') . $projectheading->title);
    echo '<br/>';
...
/************************/
function promising_print_resume($project, $currentGroupId, $fatherid, $numresume){
...
*/
function promising_get_options($domain, $projectid){
	global $DB;
	
    if (!function_exists('getLocalized')){
        function getLocalized(&$var){
            $var->label = get_string($var->label, 'promising');
            $var->description = get_string($var->description, 'promising');
        }

        function getFiltered(&$var){
            $var->label = format_string($var->label, 'promising');
            $var->description = format_string($var->description, 'promising');
        }
    }

    if (!$options = $DB->get_records_select('promising_qualifier', " domain = ? AND  projectid = ? ", array($domain,$projectid))){
    	if ($siteoptions = $DB->get_records_select('promising_qualifier', " domain = ? AND  projectid = 0 ", array($domain))){
...
*/
function promising_get_option_by_key($domain, $projectid, $value){
	global $DB;
    ...
    if (!$option = $DB->get_record('promising_qualifier', array('domain' => $domain, 'projectid' => $projectid, 'code' => $value))){
        if ($option = $DB->get_record('promising_qualifier', array('domain' => $domain, 'projectid' => 0, 'code' => $value))){
            getLocalized($option);
        } else {
           $option->id = 0;
           $option->truelabel = 'default';
           $option->label = get_string('unqualified', 'promising');
           $option->description = '';
        }
    }
    return $option;
...
*
*/
function promising_bar_graph_over($value, $over, $width = 50, $height = 4, $maxover = 60){
    global $CFG;
    
    if ($value == -1) return "<img src=\"{$CFG->wwwroot}/mod/promising/pix/p/graypixel.gif\" title=\"".get_string('nc','promising')."\" width=\"{$width}\" height=\"{$height}\" />";
    $done = floor($width * $value / 100);
    $todo = floor($width * ( 1 - $value / 100));
    $bargraph = "<img src=\"{$CFG->wwwroot}/mod/promising/pix/p/greenpixel.gif\" title=\"{$value}%\" width=\"{$done}\" height=\"{$height}\" />";
    $bargraph .= "<img src=\"{$CFG->wwwroot}/mod/promising/pix/p/bluepixel.gif\" title=\"{$value}%\" width=\"{$todo}\" height=\"{$height}\" />";
    if ($over){
        $displayOver = (round($over/$width*100))."%";
        if ($over < $maxover){
            $bargraph .= "<img src=\"{$CFG->wwwroot}/mod/promising/pix/p/redpixel.gif\" title=\"".get_string('overdone','promising').':'.$displayOver."\" width=\"{$over}\" height=\"{$height}\" />";
        } else {
            $bargraph .= "<img src=\"{$CFG->wwwroot}/mod/promising/pix/p/maxover.gif\" title=\"".get_string('overoverdone','promising').':'.$displayOver."\" height=\"{$height}\" width=\"{$width}\" />";
        }
    }    
    return $bargraph;
}
...
*/
function promising_check_task_circularity($taskid, $masterid){
	global $DB;
	
    if ($slavetasks = $DB->get_records('promising_task_dependency', array('master' => $taskid))){
        foreach($slavetasks as $aTask){
            if ($aTask->id == $masterid) return true;
            if (promising_check_task_circularity($aTask->id, $masterid)) return true;
        }
    }
    return false;
}
...
*/
function promising_print_entitycount($table1, $table2, $projectid, $groupid, $what, $relwhat, $id, $whatList = ''){
    global $CFG, $DB;

    // get concerned subtree if not provided
    if (!isset($whatList) || empty($whatList)){
	    $whatList = str_replace(",", "','", promising_get_subtree_list($table1, $id));
	}
	...
	if ($res->subs > 0 || $subcount > 0){
	    $output = "<img src=\"{$CFG->wwwroot}/mod/promising/pix/p/{$relwhat}.gif\" title=\"".get_string('bounditems', 'promising', $relwhat)."\" />(".$res->subs."){$subcount}";
	} else {
	    $output = '';
	}
	return $output;
...
*
*/
function promising_print_group_commands($additional = ''){
    global $CFG;

	$optionList[''] = get_string('choosewhat', 'promising');
    $optionList['deleteitems'] = get_string('deleteselected', 'promising');
    $optionList['copy'] = get_string('copyselected', 'promising');
    $optionList['move'] = get_string('moveselected', 'promising');
    $optionList['export'] = get_string('xmlexportselected', 'promising');
    if (!empty($additional)){
        foreach($additional as $aCommand){
            $optionList[$aCommand] = get_string($aCommand.'selected', 'promising');
        }
    }
    echo '<p>'.get_string('withchosennodes', 'promising');
    echo html_writer::select($optionList, 'cmd', '', array('' => get_string('choosewhat', 'promising')), array('onchange' => 'sendgroupdata()'));
	echo '</p>';
}
/**
 ...
 */
function promising_print_project_table($table, $return=false) {
    $output = '';
...
* @return the grade
*/
function promising_autograde($project, $groupid){
    global $CFG, $DB;

    echo $OUTPUT->heading(get_string('autograde', 'promising'));
    // get course module
    ...
    $rootRequirements = $DB->get_records_select('promising_requirement', "projectid = ? AND groupid = ? AND fatherid = 0", array($project->id, $group));
    $effectiveRequirements = array();
    foreach($rootRequirements as $aRoot){
        $effectiveRequirements = array_merge($effectiveRequirements, promising_count_leaves('promising_requirement', $aRoot->id, true));
    }
    ...
    foreach($effectiveRequirements as $aRequirement){
        $upperBranchList = promising_tree_get_upper_branch('promising_requirement', $aRequirement, true, false);
    ...
    echo '<br/><b>'.get_string('requirementsrate', 'promising').' :</b> '.$coveredReqs.' '.get_string('over', 'promising').' '.$effectiveRequirementsCount.' : '.sprintf("%.02f", $requRate);
    ...
    $rootDeliverables = $DB->get_records_select('promising_deliverable', "projectid = ? AND groupid = ? AND fatherid = 0", array($project->id, $group));
    $effectiveDeliverables = array();
    foreach($rootDeliverables as $aRoot){
        $effectiveDeliverables = array_merge($effectiveDeliverables, promising_count_leaves('promising_deliverable', $aRoot->id, true));
    }
    ...
    foreach($effectiveDeliverables as $aDeliverable){
        $upperBranchList = promising_tree_get_upper_branch('promising_deliverable', $aDeliverable, true, false);
        $upperBranchList = str_replace(',', "','", $upperBranchList);
        $query = "
           SELECT
               COUNT(*) as coveringChains
    ...
    echo '<br/><b>'.get_string('deliverablesrate', 'promising').' :</b> '.$coveredDelivs.' '.get_string('over', 'promising').' '.$effectiveDeliverablesCount.' : '.sprintf("%.02f", $delivRate);
    ...
    $rootTasks = $DB->get_records_select('promising_task', "projectid = ? AND groupid = ? AND fatherid = 0", array($project->id, $group)); 
    $completion = 0;
    if ($rootTasks){
        ...
        echo '<br/><b>'.get_string('completionrate', 'promising').' :</b> '.$done;
    }
    ...
    if ($rootTasks){
        ...
        // get leaves
        foreach($rootTasks as $aTask){
            $leafTasks = array_merge($leafTasks, promising_count_leaves('promising_task', $aTask->id, true));
        }
        // collecting and accumulating charge planned
        // get student list
        if (!groups_get_activity_groupmode($cm, $course)){
            $groupStudents = get_users_by_capability($coursecontext, 'mod/promising:canbeevaluated', 'u.id; u.firstname, u.lastname, u.mail, u.picture', 'u.lastname');
        } else {
            $groupmembers = get_group_members($groupid);
            $groupStudents = array();
            if ($groupmembers){
                foreach($groupmembers as $amember){
                    if (has_capability('mod/promising:canbeevaluated', $coursecontext, $amember->id)){
        ...
        echo '<br/><b>' . get_string('chargedispersion', 'promising') . ' :</b> ' . sprintf("%.02f", $sigma);
    }
    $totalGrade = round((0 + @$done + @$requRate + @$delivRate) / 3, 2);
    echo '<br/><b>' . get_string('mean', 'promising') . ' :</b> ' . sprintf("%.02f", $totalGrade);
    if ($project->grade > 0){
        echo '<br/><b>' . get_string('scale', 'promising') . ' :</b> ' . $project->grade;
        echo '<br/><b>' . get_string('grade', 'promising') . ' :</b> ' . round($project->grade * $totalGrade);
    }
    return $totalGrade;
}
...
*/
function promising_get_users_not_in_group($courseid){
    $coursecontext = context_course::instance($courseid);
    $users = get_users_by_capability($coursecontext, 'mod/promising:beassignedtasks', 'u.id, firstname,lastname,picture,email', 'lastname');
    if ($users){
...
* @return an array of users
*/
function promising_get_group_users($courseid, $cm, $groupid){
	global $DB;
    ...
    if (!groups_get_activity_groupmode($cm, $course)){
        ...
        if ($groupid){
            ...
            $users = promising_get_users_not_in_group($courseid);
        }
        if($users){
            ...
            foreach($users as $userid => $user){
                if (!has_capability('mod/promising:beassignedtasks', $context, $user->id)){
                    unset($users[$userid]);
                }
            }
        }
...
* @uses $COURSE
*/
function promising_get_full_xml(&$project, $groupid){
    global $COURSE, $CFG, $DB;

    include_once "xmllib.php";
    // getting heading
    $heading = $DB->get_record('promising_heading', array('projectid' => $project->id, 'groupid' => $groupid));
    $projects[$heading->projectid] = $heading;
    $xmlheading = recordstoxml($projects, 'project', '', false, null);

    // getting requirements
    promising_tree_get_tree('promising_requirement', $project->id, $groupid, $requirements, 0);
    $strengthes = $DB->get_records_select('promising_qualifier', " projectid = ? AND domain = 'strength' ", array($project->id));
    if (empty($strenghes)){
        $strengthes = $DB->get_records_select('promising_qualifier', " projectid = 0 AND domain = 'strength' ", array());
    }
    ...
    promising_tree_get_tree('promising_specification', $project->id, $groupid, $specifications, 0);
    $priorities = $DB->get_records_select('promising_qualifier', " projectid = ? AND domain = 'priority' ", array($project->id));
    if (empty($priorities)){
        $priorities = $DB->get_records_select('promising_qualifier', " projectid =  0 AND domain = 'priority' ", array());
    }
    $severities = $DB->get_records_select('promising_qualifier', " projectid = ? AND domain = 'severity' ", array($project->id));
    if (empty($severities)){
        $severities = $DB->get_records_select('promising_qualifier', " projectid = 0 AND domain = 'severity' ", array());
    }
    $complexities = $DB->get_records_select('promising_qualifier', " projectid = ? AND domain = 'complexity' ", array($project->id));
    if (empty($complexities)){
        $complexities = $DB->get_records_select('promising_qualifier', " projectid = 0 AND domain = 'complexity' ", array());
    }
    ...
    // getting tasks
    promising_tree_get_tree('promising_task', $project->id, $groupid, $tasks, 0);
    if (!empty($tasks)){
    ...
    $worktypes = $DB->get_records_select('promising_qualifier', " projectid = ? AND domain = 'worktype' ", array($project->id));
    if (empty($worktypes)){
        $worktypes = $DB->get_records_select('promising_qualifier', " projectid = 0 AND domain = 'worktype' ", array());
    }
    $taskstatusses = $DB->get_records_select('promising_qualifier', " projectid = ? AND domain = 'taskstatus' ", array($project->id));
    if (empty($taskstatusses)){
        $taskstatusses = $DB->get_records_select('promising_qualifier', " projectid = 0 AND domain = 'taskstatus' ");
    }
    ...
    promising_tree_get_tree('promising_deliverable', $project->id, $groupid, $deliverables, 0);
    $delivstatusses = $DB->get_records_select('promising_qualifier', " projectid = ? AND domain = 'delivstatus' ", array($project->id));
    if (empty($delivstatusses)){
        $delivstatusses = $DB->get_records_select('promising_qualifier', " projectid = 0 AND domain = 'delivstatus' ", array());
    }
    ...
    // getting milestones
    promising_tree_get_list('promising_milestone', $project->id, $groupid, $milestones, 0);
    $xmlmiles = recordstoxml($milestones, 'milestone', '', false, null);

    /// Finally, get the master record and make a full XML with it
    $promising = $DB->get_record('promising', array('id' => $project->id));
    $promising->wwwroot = $CFG->wwwroot;
    $promisings[$promising->id] = $promising;
    $project->xslfilter = (empty($project->xslfilter)) ? $CFG->dirroot.'/mod/promising/xsl/default.xsl' : $CFG->dataroot."/{$COURSE->id}/moddata/promising/{$project->id}/{$project->xslfilter}" ;
    $project->cssfilter = (empty($project->cssfilter)) ? $CFG->dirroot.'/mod/promising/xsl/default.css' : $CFG->dataroot."/{$COURSE->id}/moddata/promising/{$project->id}/{$project->cssfilter}" ;
    $xmlstylesheet = "<?xml-stylesheet href=\"{$project->xslfilter}\" type=\"text/xsl\"?>\n";
    $xml = recordstoxml($promisings, 'promising', $xmlheading.$xmlrequs.$xmlspecs.$xmltasks.$xmldelivs.$xmlmiles, true, null, $xmlstylesheet);
...
* @param string $sortby 
*/
function promising_get_domain($domain, $id, $how = false, $scope, $sortby = 'label'){
	global $DB;
    ...
    if (empty($id)){
        ...
        if ($how == 'menu'){
            if ($records = $DB->get_records_select_menu("promising_qualifier", " projectid = ? AND domain = ? ", array($scope, $domain), $sortby, 'id, label')){
            ...
            return $DB->get_records_select("promising_qualifier", " projectid = ? AND domain = ? ", array($scope, $domain), $sortby);
        }
    }
    if ($how == 'bycode'){
        return format_string($DB->get_field("promising_qualifier", 'label', array('domain' => $domain, 'projectid' => $scope, 'code' => $id)));
    } else {
        return format_string($DB->get_field("promising_$domain", 'label', array('domain' => $domain, 'projectid' => $scope, 'id' => $id)));
    }
}
...
*
*/
function promising_print_validations($project, $groupid, $fatherid, $cmid){
	global $CFG, $USER, $DB, $OUTPUT;

	static $level = 0;

    if ($validationsessions = $DB->get_records_select('promising_valid_session', " projectid = ? AND groupid = ? ", array($project->id, $groupid))){
		$validationcaptions = '';
		$deletestr = '<span title="'.get_string('delete').'" style="color:red">x</span>';
		$closestr = get_string('close', 'promising');
		$updatestr = get_string('update', 'promising');
	    foreach($validationsessions as $sessid => $session){
	    	$validationsessions[$sessid]->states = $DB->get_records('promising_valid_state', array('validationsessionid' => $session->id), '', 'reqid,status,comment');
	    	$validationcaption = '&lt;'.userdate($session->datecreated).'&gt;';
	    	if (has_capability('mod/promising:managevalidations', context_module::instance($cmid))){
		    	$validationcaption .= " <a href=\"{$CFG->wwwroot}/mod/promising/view.php?id=$cmid&view=validations&work=dodelete&validid={$sessid}\">$deletestr</a>";
		    }
	    	if ($session->dateclosed == 0){
	    		if (has_capability('mod/promising:managevalidations', context_module::instance($cmid))){
		    		$validationcaption .= " <a href=\"{$CFG->wwwroot}/mod/promising/view.php?id=$cmid&view=validations&work=close&validid={$sessid}\">$closestr</a>";
		    	}
	    		if (has_capability('mod/promising:validate', context_module::instance($cmid))){
		    		$validationcaption .= " <a href=\"{$CFG->wwwroot}/mod/promising/view.php?id=$cmid&view=validation&validid={$sessid}\">$updatestr</a>";
		    	}
		    }
		...
		if (!empty($project->projectusesrequs)){
			...
			print_error('errornovalidatingentity', 'promising');
		}
		$query = "
		    SELECT 
		        e.*,
		...
		if ($entities = $DB->get_records_sql($query)) {
			...
			foreach($entities as $entity){
			    echo "<div class=\"nodelevel{$level}\">";
			    $level++;
			    promising_print_single_entity_validation($validationsessions, $entity, $project, $groupid, $cmid, count($entities), $entityname);
	            $visibility = ($entity->collapsed) ? 'display: none' : 'display: block' ; 
				echo "<div id=\"sub{$entity->id}\" class=\"treenode\" style=\"$visibility\" >";
				promising_print_validations($project, $groupid, $entity->id, $cmid);
				echo "</div>";
			    $level--;
				echo "</div>";
			}
			...
			if ($level == 0){
				echo $OUTPUT->box_start();
				print_string('emptyproject', 'promising');
				echo $OUTPUT->box_end();
			}
		}
	} else {
		if ($level == 0){
			echo $OUTPUT->box_start();
			print_string('novalidationsession', 'promising');
			echo $OUTPUT->box_end();
		}
	}
}
...
* @param fullsingle true if prints a single isolated element
*/
function promising_print_single_entity_validation(&$validationsessions, &$entity, &$project, $group, $cmid, $countentities, $entityname){
    global $CFG, $USER, $DB;

    static $classswitch = 'even';

    $context = context_module::instance($cmid);
    $canedit = has_capability('mod/promising:validate', $context);
    $numrec = implode('.', promising_tree_get_upper_branch('promising_'.$entityname, $entity->id, true, true));
	if (promising_count_subs('promising_'.$entityname, $entity->id) > 0) {
		$hidesub = "<a href=\"javascript:toggle('{$entity->id}','sub{$entity->id}');\"><img name=\"img{$entity->id}\" src=\"{$CFG->wwwroot}/mod/promising/pix/p/switch_minus.gif\" alt=\"collapse\" /></a>";
	} else {
		$hidesub = "<img src=\"{$CFG->wwwroot}/mod/promising/pix/p/empty.gif\" />";
	}
    $validations = $DB->get_records_select('promising_valid_state', " projectid = ? AND groupid = ? AND reqid = ? ", array($project->id, $group, $entity->id));
	...
	foreach($validationsessions as $session){
		if (!isset($session->states[$entity->id])){
			...
			$newstate->comment = '';
			$stateid = $DB->insert_record('promising_valid_state', $newstate);
		...
		$validationcells .= '<span title="'.$session->states[$entity->id]->comment.'">'.get_string(strtolower($session->states[$entity->id]->status), 'promising').'</span>';
		$validationcells .= '</td>';
	}
...
function promising_print_validation_states_form($validsessid, &$project, $groupid, $fatherid = 0, $cmid = 0){
	global $CFG, $USER, $DB;
	static $level = 0;
	...
	if ($states = $DB->get_records_sql($sql)){
		...
		foreach($states as $state){
		    echo "<div class=\"nodelevel{$level}\">";
		    $level++;
		    promising_print_single_validation_form($state, $entityname);

            $visibility = ($state->collapsed) ? 'display: none' : 'display: block' ; 
			echo "<div id=\"sub{$state->reqid}\" class=\"treenode\" style=\"$visibility\" >";
			promising_print_validation_states_form($validsessid, $project, $groupid, $state->reqid, $cmid);
			echo "</div>";
		    $level--;
			echo "</div>";
		}
...
*
*/
function promising_print_single_validation_form($state, $entityname){
	global $CFG;

    $numentity = implode('.', promising_tree_get_upper_branch('promising_'.$entityname, $state->reqid, true, true));
	if (promising_count_subs('promising_'.$entityname, $state->reqid) > 0) {
		$hidesub = "<a href=\"javascript:toggle('{$state->reqid}','sub{$state->reqid}');\"><img name=\"img{$state->reqid}\" src=\"{$CFG->wwwroot}/mod/promising/pix/p/switch_minus.gif\" alt=\"collapse\" /></a>";
	} else {
		$hidesub = "<img src=\"{$CFG->wwwroot}/mod/promising/pix/p/empty.gif\" />";
	}
	...
	$validationstatus['UNTRACKED'] = get_string('untracked', 'promising');
	$validationstatus['REFUSED'] = get_string('refused', 'promising');
	$validationstatus['MISSING'] = get_string('missing', 'promising');
	$validationstatus['BUGGY'] = get_string('buggy', 'promising');
	$validationstatus['TOENHANCE'] = get_string('toenhance', 'promising');
	$validationstatus['ACCEPTED'] = get_string('accepted', 'promising');
	$validationstatus['REGRESSION'] = get_string('regression', 'promising');
...
function milestone_checkConstraints($project, $milestone){
    ...
    if($latestTask && $milestone->deadline < $latestTask->latest){
        $control['milestonedeadline'] = get_string('assignedtaskendsafter','promising') . '<br/>' . userdate($latestTask->latest);
    }
    return $control;
}
    ...
    */
    function task_checkConstraints($project, $task){
        ...
        if ($task->taskstartenable && $task->taskstart < $project->projectstart){
            $control['taskstartdate'] = get_string('tasktoosoon','promising') . '<br/>' . userdate($project->projectstart);
        }
        // task too late (planned to milestone)
        if($task->taskstartenable && $task->milestoneid){
            $milestone = $DB->get_record('promising_milestone', array('projectid' => $project->id, 'id' => $task->milestoneid));
            if ($milestone->deadlineenable && ($task->taskstart + $plannedtime > $milestone->deadline)){
                $control['taskstartdate'] = get_string('taskstartsaftermilestone','promising') . '<br/>' . userdate($milestone->deadline);
            }
        }
        // task too late (absolute)
        elseif($task->taskstartenable && ($task->taskstart + $plannedtime > $project->projectend)){
            $control['taskstartdate'] = get_string('tasktoolate','promising') . '<br/>' . userdate($project->projectend);
        }
        ...
        elseif($task->taskendenable && $task->milestoneid){
            ...
            if ($milestone->deadlineenable && ($task->taskend > $milestone->deadline)){
                $control['taskenddate'] = get_string('taskfinishesaftermilestone','promising') . '<br/>' . userdate($milestone->deadline);
            }
        }
        // checking too late end
        elseif($task->taskendenable && $task->taskend > $project->projectend){
            $control['taskenddate'] = get_string('taskfinishestoolate','promising') . '<br/>' . userdate($project->projectend);
        }
        // checking switched end and start
        elseif($task->taskendenable && $task->taskstartenable && $task->taskend <= $task->taskstart){
            $control['taskenddate'] = get_string('taskfinishesbeforeitstarts','promising');
        }
        // checking unfeseabletask
        elseif($task->taskendenable && $task->taskstartenable && $task->taskend < $task->taskstart + $plannedtime){
            $control['taskenddate'] = get_string('tasktooshort','promising') . '<br/> >> ' . userdate($task->taskstart + $plannedtime);
        }
        return $control;
    }

function promising_check_startup_level($entity, $fatherid, &$level, &$startuplevelcheck){
	global $DB;
	...
	if (!$startuplevelcheck){
		if (!$fatherid){
			...
			while($rec->fatherid){
				$rec = $DB->get_record('promising_'.$entity, array('id' => $rec->fatherid), 'id, fatherid');
				$level++;
			}
		}
...
function promising_print_localfile($deliverable, $cmid, $type=NULL, $align="left") {
	global $CFG, $DB, $OUTPUT;
	    ...
	    if ($files = $fs->get_area_files($context->id, 'mod_promising', 'localfile', $deliverable->id, "timemodified", false)) {
	        foreach ($files as $file) {
	            $filename = $file->get_filename();
	            $mimetype = $file->get_mimetype();
	            $iconimage = '<img src="'.$OUTPUT->pix_url(file_mimetype_icon($mimetype)).'" class="icon" alt="'.$mimetype.'" />';
	            $path = file_encode_url($CFG->wwwroot.'/pluginfile.php', '/'.$context->id.'/mod_promising/localfile/'.$deliverable->id.'/'.$filename);
...
function promising_print_messages($project, $group, $cmid, $parent, $propagated=null){
	global $CFG, $USER, $DB, $OUTPUT;
	echo "<h2>".get_string('disculiste','promising')."</h2>";
	$query = "
	    SELECT 
	        d.*
        FROM 
            {promising_messages} as d
	    WHERE 
	        d.groupid = {$group} AND 
	        d.projectid = {$project->id} AND 
	        d.parent = {$parent}
	...
	if ($discussions = $DB->get_records_sql($query)) {
		...
		<thead>
			<tr>
			  <th class='header'>".get_string('discu','promising')."</th>
			  <th class='header' colspan='2'>".get_string('discupar','promising')."</th>
			  <th class='header'>".get_string('nbreponse','promising')."</th>
			  <th class='header'>".get_string('derniermessage','promising')."</th>
			</tr>
		</thead>";
		foreach($discussions as $discussion){
		//affichages des discussions 
            promising_print_single_message($discussion, $project, $group, $cmid, count($discussions));
		}
		echo "</table>";
	} else {
		echo "<p>".get_string('nodiscu','promising')."</p>";
	}
}

//fonction d'affichage d'une discussion
function promising_print_single_message($discussion, $project, $group, $cmid, $setSize){
    global $CFG, $USER, $DB, $OUTPUT;
	//$status = $DB->get_record('promising_messages', array('id' => $discussion->id));
	$context = context_module::instance($cmid);
    ...
    $canedit = $USER->editmode == 'on' && has_capability('mod/promising:changediscussion', $context);
	if ($canedit) {
	    $actionsDiscu='';
...
function promising_print_projects($project, $group, $cmid){
	global $CFG, $USER, $DB, $OUTPUT;
	if($project->projectgrpid==0){
		...
		echo "<h3 id='vueprojetstitle'><img src='".$OUTPUT->pix_url('vue-projets', 'promising')."' alt='' />".get_string('vueprojetstitle', 'promising').$roleprojectgrp->name."</h3>";
		if ($projects = $DB->get_records_sql($query)) {
			if(count($projects>0)){
				...
				foreach($projects as $projectitem){
					...
					if($cmtmp = get_coursemodule_from_instance('promising', $projectitem->id)){ //parfois impossible de récuperer le cm associé au projet... et donc pas de lien possible
						$projectName = "<a href=\"".$CFG->wwwroot."/mod/promising/view.php?id=".$cmtmp->id."&view=description"."\" title=\"Voir ce projet\">".$projectitem->name."</a>";
					}
					...
					if($projectitem->etat==0 && $projetfini){// etat 0 = projet pas fermé
						echo "<p><span style=\"cursor:pointer;display:inline;\" onclick=\"javascript:var r=confirm('Etes vous sure de vouloir clore le projet ".addslashes($projectitem->name)." ?');if (r==true){window.location.href='{$CFG->wwwroot}/mod/promising/view.php?id={$cmid}&amp;view=projects&amp;work=close&amp;projectid=".$projectitem->id."';}\">Clore le projet</span></p>";
...
*/
function promising_print_projects_xml($projectgrpid){
    global $CFG, $DB;
	
	$xml = new DOMDocument('1.0', 'utf-8');
	
	$recs = $DB->get_records('promising', array('projectgrpid' => $projectgrpid,'etat' => 1));
